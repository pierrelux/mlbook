...
**Non-équivalence pour Adam.** Avec Adam, la distinction devient concrète. Si l'on applique la régularisation L2, le terme $\lambda\boldsymbol{\theta}$ est ajouté au gradient *avant* le calcul des moments et la normalisation adaptative. Ce terme est donc divisé par $\sqrt{\hat{\mathbf{s}}}$ comme le reste du gradient: les paramètres dont les gradients sont grands (et donc $\hat{\mathbf{s}}$ est grand) reçoivent *moins* de régularisation. L'effet de la pénalité L2 est atténué de façon non uniforme par la normalisation adaptative, ce qui n'est pas l'intention.
...
Chaque paramètre est contracté par le même facteur, indépendamment de l'historique de ses gradients. **AdamW** {cite}`loshchilov2019decoupled` implémente cette variante et est devenu le choix standard pour l'entraînement de grands modèles.
