...
Le **rang effectif** est le nombre de valeurs singulières significativement au-dessus du plancher de bruit. Dans cet exemple, nous avons simulé 5 facteurs latents, et le spectre révèle clairement cette structure: les 5 premières valeurs singulières dominent.
...
Notre estimateur $\hat{f}$ est entraîné sur un échantillon aléatoire $\mathcal{D}$. Comme $\mathcal{D}$ est aléatoire, $\hat{f}(\mathbf{x})$ est une variable aléatoire pour chaque $\mathbf{x}$ fixé. L'erreur quadratique moyenne, moyennée sur les échantillons d'entraînement possibles et le bruit, se décompose comme suit:
