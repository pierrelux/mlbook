...
L'**entropie** d'une distribution discrète $p$ mesure l'incertitude moyenne associée à un tirage aléatoire:

$$
\mathbb{H}(p) = -\sum_y p(y) \log p(y)
$$

On peut y penser comme une mesure de «surprise moyenne». Une distribution concentrée sur quelques valeurs a une faible entropie: les tirages sont prévisibles. À l'opposé, une distribution uniforme maximise l'entropie: chaque tirage est également surprenant. L'entropie est toujours positive ou nulle, et atteint zéro uniquement pour une distribution déterministe.
...
Comment quantifier à quel point deux distributions diffèrent? La **divergence de Kullback-Leibler** (ou divergence KL) répond à cette question:

$$
D_{\text{KL}}(p \| q) = \sum_y p(y) \log \frac{p(y)}{q(y)} = \mathbb{E}_{y \sim p}\left[\log \frac{p(y)}{q(y)}\right]
$$

On peut l'interpréter comme le «coût informationnel» d'utiliser la distribution $q$ pour encoder des données qui suivent réellement la distribution $p$. Ce coût est toujours positif ou nul (c'est l'inégalité de Gibbs), et il s'annule si et seulement si $p$ et $q$ sont identiques. Contrairement à une vraie distance, la divergence KL n'est pas symétrique: en général, $D_{\text{KL}}(p \| q) \neq D_{\text{KL}}(q \| p)$.

La divergence KL se décompose en faisant apparaître l'**entropie croisée** $\mathbb{H}_{\text{ce}}(p, q) = -\sum_y p(y) \log q(y)$:

$$
D_{\text{KL}}(p \| q) = \mathbb{H}_{\text{ce}}(p, q) - \mathbb{H}(p)
$$

L'entropie croisée mesure le nombre moyen de bits nécessaires pour encoder des données de $p$ en utilisant un code optimisé pour $q$. La divergence KL est l'excédent par rapport au code optimal pour $p$.

### Le maximum de vraisemblance minimise la divergence KL

Nous pouvons maintenant faire le lien avec l'EMV. Définissons la **distribution empirique**, qui place une masse $1/N$ sur chaque observation:

$$
p_{\mathcal{D}}(y) = \frac{1}{N} \sum_{i=1}^N \delta(y - y_i)
$$

Considérons la divergence KL entre cette distribution empirique et notre modèle paramétrique $p(y | \boldsymbol{\theta})$:
...
Le terme $\mathbb{H}(p_{\mathcal{D}})$ ne dépend pas de $\boldsymbol{\theta}$: c'est une propriété des données, pas du modèle. Minimiser la divergence KL revient donc à minimiser l'entropie croisée. Or cette entropie croisée avec la distribution empirique n'est autre que la log-vraisemblance négative:

$$
\mathbb{H}_{\text{ce}}(p_{\mathcal{D}}, p(\cdot|\boldsymbol{\theta})) = -\frac{1}{N} \sum_{i=1}^N \log p(y_i | \mathbf{x}_i; \boldsymbol{\theta}) = \frac{1}{N}\text{LVN}(\boldsymbol{\theta})
$$

Le maximum de vraisemblance trouve donc les paramètres qui minimisent la divergence KL entre notre modèle et la distribution empirique. Géométriquement, nous cherchons, dans la famille de distributions paramétrées par $\boldsymbol{\theta}$, celle qui est la plus «proche» des données observées.
