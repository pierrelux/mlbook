...
Le membre de gauche ne dépend pas de $\mathbf{Z}$. En prenant l'espérance sous $q(\mathbf{Z})$, on obtient donc la même quantité:

$$
\log p(\mathbf{X} \mid \boldsymbol{\theta}) = \mathbb{E}_{q(\mathbf{Z})}\left[ \log p(\mathbf{X}, \mathbf{Z} \mid \boldsymbol{\theta}) - \log p(\mathbf{Z} \mid \mathbf{X}, \boldsymbol{\theta}) \right].
$$
