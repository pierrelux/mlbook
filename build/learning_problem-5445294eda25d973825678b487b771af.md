...
## Lien entre MRE et le cadre probabiliste

La minimisation du risque empirique et l'inférence probabiliste sont deux perspectives sur le même problème. Le tableau suivant résume les correspondances:

| Approche décisionnelle (MRE) | Approche probabiliste |
|------------------------------|----------------------|
| Risque empirique | Log-vraisemblance négative (NLV) |
| Risque empirique régularisé | Log-postérieur négatif |
| Minimiseur du risque empirique | Estimateur du maximum de vraisemblance (EMV) |
| Minimiseur du risque régularisé | Estimateur du maximum a posteriori (MAP) |

### MRE et maximum de vraisemblance

Lorsque nous choisissons la perte logarithmique $\ell(y, f(\mathbf{x})) = -\log p(y | f(\mathbf{x}))$, le risque empirique devient:

$$
\hat{\mathcal{R}}(\boldsymbol{\theta}) = \frac{1}{N} \sum_{i=1}^N -\log p(y_i | \mathbf{x}_i; \boldsymbol{\theta}) = \frac{1}{N} \text{NLV}(\boldsymbol{\theta})
$$

Le minimiseur du risque empirique est donc l'estimateur du maximum de vraisemblance.

### MRE régularisé et MAP

Lorsque nous ajoutons un terme de régularisation $C(\boldsymbol{\theta}) = -\log p(\boldsymbol{\theta})$, le risque empirique régularisé devient:

$$
\hat{\mathcal{R}}_\lambda(\boldsymbol{\theta}) = \frac{1}{N} \text{NLV}(\boldsymbol{\theta}) + C(\boldsymbol{\theta}) = -\frac{1}{N} \log p(\mathcal{D}|\boldsymbol{\theta}) - \log p(\boldsymbol{\theta})
$$

À une constante près, c'est le logarithme négatif de la distribution a posteriori. Le minimiseur est l'estimateur MAP.
...
Cette quantité est toujours positive ou nulle, et vaut zéro si et seulement si $p = q$. Elle n'est pas symétrique.
...
Le premier terme, l'entropie de la distribution empirique, ne dépend pas de $\boldsymbol{\theta}$. Minimiser la divergence KL revient à minimiser la NLV:
