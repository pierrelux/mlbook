...
La **saturation** est le mécanisme principal de la dissolution du gradient. La dérivée de la sigmoïde (vue au chapitre 7) est $\sigma'(a) = \sigma(a)(1 - \sigma(a))$, bornée par 0,25 (atteinte en $a = 0$). Pour les grandes valeurs de $|a|$, la dérivée est proche de zéro: la fonction est saturée et le gradient s'annule.

Dans un réseau de $L$ couches sigmoïdes, chaque couche multiplie le gradient par un facteur d'au plus 0,25. Après $L$ couches, la norme du gradient est au plus $0{,}25^L$ fois sa valeur initiale. Pour $L = 20$, cela donne $(0{,}25)^{20} \approx 10^{-12}$: le gradient est pratiquement nul.
