...
**Qu'est-ce que la SVD?** La SVD est un outil qui décompose une matrice en composantes plus simples, un peu comme on peut décomposer un nombre en facteurs premiers. Pour une matrice $\mathbf{X}$ de données, la SVD la réécrit comme le produit de trois matrices:

$$
\mathbf{X} = \mathbf{U} \mathbf{D} \mathbf{V}^\top
$$

Chacune de ces matrices a une interprétation géométrique:

- **$\mathbf{V}$** contient les **directions principales** dans l'espace des caractéristiques (les colonnes $\mathbf{v}_j$ sont des vecteurs orthonormaux). Ces directions indiquent les axes le long desquels les données varient le plus.
- **$\mathbf{D}$** est une matrice diagonale contenant les **valeurs singulières** $d_1 \geq d_2 \geq \cdots \geq d_d \geq 0$, ordonnées du plus grand au plus petit. Chaque valeur singulière $d_j$ mesure l'**importance** ou la **force** de la direction correspondante $\mathbf{v}_j$. Une grande valeur singulière signifie que les données varient beaucoup le long de cette direction; une petite valeur singulière signifie que la variation est faible.
- **$\mathbf{U}$** contient les directions correspondantes dans l'espace des observations (les colonnes $\mathbf{u}_j$ sont orthonormales).

**Pourquoi les valeurs singulières sont-elles importantes?** Imaginez que vous regardez un nuage de points en 2D. Si les points s'étirent beaucoup plus dans une direction que dans l'autre, la première direction a une grande valeur singulière et la seconde une petite. Les valeurs singulières nous disent donc quelles directions dans l'espace des caractéristiques sont les plus "informatives" pour expliquer les données.
...
Cette formule décompose la solution en une somme de contributions le long de chaque direction principale $\mathbf{v}_j$. Le terme $\frac{\mathbf{u}_j^\top \mathbf{y}}{d_j}$ mesure combien la sortie $\mathbf{y}$ s'aligne avec la direction $\mathbf{u}_j$, divisé par l'importance $d_j$ de cette direction. Notez que diviser par une petite valeur singulière $d_j$ peut amplifier le bruit, ce qui explique pourquoi MCO peut être instable quand certaines directions sont peu informatives.
