...
Commençons par un idéal théorique. Si nous connaissions la **vraie** distribution conditionnelle $p(y|\mathbf{x})$ (celle de la nature, pas un modèle appris), quelle serait la meilleure prédiction possible? Le **prédicteur de Bayes optimal** minimise la perte espérée:

$$
f^*(\mathbf{x}) = \arg\min_{\hat{y}} \mathbb{E}_{p(y|\mathbf{x})}[\ell(y, \hat{y})]
$$

Pour la **perte quadratique**, le prédicteur optimal est la moyenne conditionnelle $f^*(\mathbf{x}) = \mathbb{E}[y | \mathbf{x}]$. Pour la **perte 0-1** en classification, c'est le mode conditionnel $f^*(\mathbf{x}) = \arg\max_c \, p(y = c | \mathbf{x})$, la classe la plus probable.

Ce prédicteur est un **repère théorique**: aucun algorithme ne peut faire mieux, car il utilise la vraie distribution. La différence entre le risque d'un prédicteur appris et le risque de Bayes mesure ce que nous perdons en ne connaissant pas la vraie distribution.
