...
Les sections précédentes ont présenté le MLP comme un objet mathématique: une composition de transformations affines et de non-linéarités. Nous avons dérivé la rétropropagation, exploré les algorithmes d'optimisation, et implémenté un réseau complet en NumPy. Mais à quoi ressemble un MLP utilisé pour un problème réel? La réponse dépend de la nature des entrées et des sorties, et elle nous ramène directement aux modèles linéaires des chapitres 2 et 3.

### Régression avec un MLP

Au chapitre 2, nous avons vu que la régression linéaire prédit la moyenne d'une gaussienne conditionnelle:

$$
\hat{y} = \boldsymbol{\theta}^\top \mathbf{x}
$$

La sortie est un scalaire, il n'y a pas de fonction d'activation, et la perte est la somme des carrés. Passer à un MLP revient à insérer des couches cachées entre l'entrée et cette sortie linéaire. La dernière couche reste identique: pas d'activation, perte quadratique.

```{mermaid}
graph LR
    x["x ∈ ℝ<sup>d</sup>"]
    h1["Couche cachée<br/>h neurones, ReLU"]
    h2["Couche cachée<br/>h neurones, ReLU"]
    y["ŷ ∈ ℝ<br/>(linéaire)"]
...
Pour un problème concret avec $d = 10$ variables en entrée, on pourrait choisir deux couches cachées de 64 neurones avec ReLU, suivies d'un seul neurone de sortie sans activation. La structure est la même que la régression linéaire, mais les couches cachées construisent une représentation $\boldsymbol{\phi}(\mathbf{x}; \boldsymbol{\theta}_\phi)$ dans laquelle la relation avec $y$ est (presque) linéaire.

Si l'on veut prédire un vecteur $\mathbf{y} \in \mathbb{R}^K$ (par exemple des coordonnées en 2D, ou plusieurs cibles simultanément), il suffit d'augmenter le nombre de neurones de sortie à $K$, tous linéaires. La perte devient la somme des carrés sur les $K$ sorties: $\sum_{k=1}^K (y_k - \hat{y}_k)^2$.

### Classification avec un MLP

Au chapitre 3, la régression logistique modélise la probabilité de la classe positive:

$$
p(y = 1 | \mathbf{x}) = \sigma(\boldsymbol{\theta}^\top \mathbf{x})
$$

Un MLP pour la classification binaire garde cette même couche de sortie: un neurone suivi d'une sigmoïde, avec l'entropie croisée binaire comme perte. Les couches cachées précèdent cette couche finale.
...
Pour la classification multiclasse avec $K$ classes, la sortie comporte $K$ neurones suivis d'un softmax, et la perte est l'entropie croisée catégorielle:
...
graph LR
    ...
    h1["Couche cachée<br/>h neurones, ReLU"]
    h2["Couche cachée<br/>h neurones, ReLU"]
    y["p(y=k|x)<br/>K neurones, softmax"]
...
Dans tous les cas, le réseau se décompose en deux parties: un extracteur de caractéristiques $\boldsymbol{\phi}(\mathbf{x}; \boldsymbol{\theta}_\phi)$ formé par les couches cachées, et une tête linéaire $\mathbf{w}^\top \boldsymbol{\phi} + b$ qui effectue la prédiction. C'est exactement l'équation {eq}`eq:nn-key-idea` du début du chapitre. Les modèles linéaires des chapitres 2 et 3 sont le cas particulier où l'extracteur de caractéristiques est l'identité: $\boldsymbol{\phi}(\mathbf{x}) = \mathbf{x}$.

### Limites du MLP

Le MLP traite son entrée comme un vecteur plat. Chaque neurone d'entrée est connecté à chaque neurone de la couche suivante, sans notion de structure spatiale ou temporelle. Pour des données tabulaires (âge, revenu, nombre de pièces), c'est approprié: il n'y a pas d'ordre naturel entre les variables.

Mais pour une image de $28 \times 28$ pixels, le MLP la transforme en un vecteur de 784 entrées. Il ne sait pas que le pixel $(0, 0)$ est voisin du pixel $(0, 1)$ mais éloigné du pixel $(27, 27)$. Pour une phrase de 10 mots, le MLP a besoin d'une entrée de taille fixe. Que fait-on avec une phrase de 20 mots? Et comment traiter une collection de phrases de longueurs différentes?
...
````{admonition} Exercice 15: de l'expression au graphe (fan-out simple) ★
:class: hint dropdown
...
````{admonition} Exercice 17: de l'expression au graphe (triple fan-out) ★★
:class: hint dropdown
...
**(c)** Pour calculer le gradient, on peut multiplier de droite à gauche. Définissons la fonction $\text{vjp}_g(a, \bar{u})$ d'une opération $g$ comme $\bar{u} \cdot J_g(a)$, c'est-à-dire le produit de l'adjoint entrant par la jacobienne locale. En partant de $\bar{v}_2 = 1$, écrivez le calcul du gradient comme une composition d'appels à $\text{vjp}$:

$$
\bar{x} = \text{vjp}_{\sin}\!\big(x,\; \text{vjp}_{\exp}(\ldots,\; \ldots)\big)
$$
...
$$
\bar{x} = \text{vjp}_{\sin}\!\big(x, \;\text{vjp}_{\exp}(v_1,\; \underbrace{1}_{\bar{v}_2})\big)
$$

**(d)** Développons:

- $\text{vjp}_{\exp}(v_1, 1) = 1 \cdot J_{\exp}(v_1) = e^{v_1} = e^{\sin(x)}$
- $\text{vjp}_{\sin}(x, e^{\sin(x)}) = e^{\sin(x)} \cdot J_{\sin}(x) = e^{\sin(x)} \cdot \cos(x)$
...
Passe arrière: $\text{vjp}_{\exp}(0, 1) = e^0 = 1$, puis $\text{vjp}_{\sin}(0, 1) = 1 \cdot \cos(0) = 1$.

Donc $f'(0) = 1$.
````

````{admonition} Exercice 19: jacobiennes et accumulation (fan-out) ★★
:class: hint dropdown
...
$$
\bar{v}_2, \bar{v}_3 = \text{vjp}_{\text{add}}(v_2, v_3,\; \bar{v}_4) \quad \text{puis} \quad \bar{v}_1 = \text{vjp}_{\exp}(v_1,\; \bar{v}_2) + \text{vjp}_{\text{sq}}(v_1,\; \bar{v}_3)
$$
...
- $\bar{v}_2, \bar{v}_3 = \text{vjp}_{\text{add}}(v_2, v_3,\; 1)$. Comme $J_{\text{add}} = [1, 1]$, on obtient $\bar{v}_2 = 1$, $\bar{v}_3 = 1$.
- Accumulation sur $v_1$:

$$
\bar{v}_1 = \text{vjp}_{\exp}(v_1,\; \bar{v}_2) + \text{vjp}_{\text{sq}}(v_1,\; \bar{v}_3) = \bar{v}_2 \cdot e^{v_1} + \bar{v}_3 \cdot 2v_1 = e^{v_1} + 2v_1
$$

- $\bar{x}, \bar{y} = \text{vjp}_{\text{sub}}(x, y,\; \bar{v}_1)$. Comme $J_{\text{sub}} = [1, -1]$:
...
**(a)** Écrivez la jacobienne de chaque opération élémentaire. Quelles variables d'entrée ont un fan-out?
...
Les deux variables $x$ et $y$ ont un fan-out: chacune alimente $v_1$ (addition) et $v_3$ (soustraction).

**(b)** Passe arrière:

- Initialiser $\bar{v}_4 = 1$.
- $\bar{v}_2, \bar{v}_3 = \text{vjp}_{\text{mul}}(v_2, v_3,\; 1) = (v_3, \; v_2)$
- $\bar{v}_1 = \text{vjp}_{\text{relu}}(v_1,\; \bar{v}_2) = \mathbb{1}[v_1 > 0] \cdot \bar{v}_2$

Accumulation sur $x$ et $y$ (deux contributions chacun):

$$
\bar{x} = \text{vjp}_{\text{add}}(x, y,\; \bar{v}_1)_x + \text{vjp}_{\text{sub}}(x, y,\; \bar{v}_3)_x = \bar{v}_1 + \bar{v}_3
$$

$$
\bar{y} = \text{vjp}_{\text{add}}(x, y,\; \bar{v}_1)_y + \text{vjp}_{\text{sub}}(x, y,\; \bar{v}_3)_y = \bar{v}_1 + (-\bar{v}_3)
$$
...
Passe arrière: $\bar{v}_4 = 1$. $\text{vjp}_{\text{mul}} \to \bar{v}_2 = 2$, $\bar{v}_3 = 4$. $\text{vjp}_{\text{relu}} \to \bar{v}_1 = \mathbb{1}[4 > 0] \cdot 2 = 2$.
...
Passe arrière: $\bar{v}_4 = 1$. $\text{vjp}_{\text{mul}} \to \bar{v}_2 = -4$, $\bar{v}_3 = 0$. $\text{vjp}_{\text{relu}} \to \bar{v}_1 = \mathbb{1}[-2 > 0] \cdot (-4) = 0$.
