...
Les deux panneaux montrent les mêmes données (les points sont colorés selon leur vrai groupe) avec les ellipses de covariance à 2 écarts-types, et les points mal classés sont cerclés de noir. À gauche, k-moyennes trace toujours la **médiatrice** du segment reliant les deux centroïdes: cette frontière droite coupe les ellipses et mal-classe les points qui se trouvent du «mauvais côté» de la perpendiculaire. À droite, le GMM ajuste une covariance propre à chaque composant, ce qui produit une frontière **courbe** qui épouse la forme des groupes et réduit le nombre d'erreurs. Pour s'adapter à cette géométrie, il faut un modèle capable de représenter des covariances différentes par groupe.
