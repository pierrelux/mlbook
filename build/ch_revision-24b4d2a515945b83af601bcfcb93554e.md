...
Ce chapitre est un guide de révision pour l'examen final. La première partie rappelle les concepts pré-intra sous forme condensée. La seconde partie approfondit la matière post-intra, qui a plus de poids à l'examen. Des questions de pratique avec solutions accompagnent chaque thème.
...
```{admonition} Concept clé — Risque et prédicteur de Bayes
:class: tip
...
La régression logistique est le modèle discriminatif central du cours. Le softmax généralise la sigmoïde au cas multiclasse. La perte associée est l'entropie croisée, dérivée du maximum de vraisemblance.

```{admonition} Concept clé — Régression logistique et softmax
:class: tip
...
Le compromis biais-variance est le fil conducteur de la première moitié du cours. Un modèle trop simple sous-apprend (biais élevé); un modèle trop complexe surapprend (variance élevée). La validation croisée guide le choix des hyperparamètres.

```{admonition} Concept clé — Biais-variance
:class: tip
...
Trois perspectives sur l'apprentissage convergent vers le même algorithme. Le MAP relie la régularisation à un a priori bayésien, et la théorie de l'information fournit une troisième lecture via la divergence KL.

```{admonition} Concept clé — Trois perspectives, un algorithme
:class: tip
...
Les modèles génératifs modélisent $p(\mathbf{x} \mid y)p(y)$ puis appliquent Bayes pour classifier, contrairement aux modèles discriminatifs qui modélisent directement $p(y \mid \mathbf{x})$. L'algorithme EM est la méthode d'estimation pour les modèles à variables latentes.

```{admonition} Concept clé — Modèles génératifs et EM
:class: tip
...
```{admonition} Concept clé — Perceptron multicouche
:class: tip
...
La dérivation automatique (AD) calcule les dérivées exactes d'un programme en appliquant la règle de la chaîne sur son graphe de calcul. C'est le mécanisme qui rend l'entraînement des réseaux de neurones possible.

#### Mode avant vs mode arrière

Pour une composition $f = f_L \circ \cdots \circ f_1$, le jacobien est $J_f = J_{f_L} \cdots J_{f_1}$. La question est : dans quel sens évaluer ce produit matriciel?
...
```{admonition} Concept clé — Mode avant vs mode arrière
:class: tip
...
- **Pourquoi le mode arrière?** Pour $\mathcal{L} : \R^n \to \R$ (1 sortie, $n$ paramètres), le mode arrière donne le gradient complet $\nabla_{\boldsymbol{\theta}} \mathcal{L}$ en **une seule passe**, peu importe $n$. Le mode avant nécessiterait $n$ passes.
```
...
```{admonition} Concept clé — Règles VJP
:class: tip
...
```{admonition} Concept clé — Le traceur en trois étapes
:class: tip
...
```{admonition} Concept clé — Optimiseurs
:class: tip
...
Dans un réseau à $L$ couches, le gradient par rapport aux premières couches est un produit de jacobiens. Si le rayon spectral de chaque jacobien est $< 1$, le gradient disparaît exponentiellement. Les quatre solutions forment un ensemble cohérent :

```{admonition} Concept clé — Stabiliser le gradient en profondeur
:class: tip
...
```{admonition} Concept clé — Trois techniques de régularisation
:class: tip
...
```{admonition} Concept clé — Auto-encodeurs
:class: tip
...
```{admonition} Concept clé — RNN et dissolution du gradient
:class: tip
...
Le mécanisme d'attention contourne les limites des RNN : chaque position accède directement à toutes les autres via des requêtes, clés et valeurs apprises.
...
```{admonition} Concept clé — Attention
:class: tip
...
```{admonition} Concept clé — Masque causal et KV cache
:class: tip
...
```{admonition} Concept clé — Gradient boosting et ensembles
:class: tip
...
graph LR
    ...
    style mul fill:#f0f0f0,stroke:#444
    style add fill:#f0f0f0,stroke:#444
    style relu fill:#f0f0f0,stroke:#444
~~~
...
**(b)** **Dissolution du gradient.** Pour $T = 100$, la rétropropagation à travers le temps (BPTT) produit un produit de 99 jacobiens $\prod_{j} \text{diag}(\tanh'(\mathbf{a}_j)) W_{hh}$. Comme $\tanh'(a) \leq 1$, ce produit décroît exponentiellement avec $T$ si les valeurs propres de $W_{hh}$ sont inférieures à 1. Les premières positions de la séquence ne reçoivent essentiellement aucun signal de gradient.
...
Voici une visualisation de la matrice de scores et de sa version masquée :
...
**(a)** Un étudiant entraîne un MLP à 20 couches avec des activations sigmoïdes et observe que les gradients des premières couches sont essentiellement nuls. Expliquez la cause et proposez deux modifications.
...
Quand $\mathbf{f}_t \approx 1$, le terme dominant est $\text{diag}(\mathbf{f}_t) \approx I$, donc $\frac{\partial \mathbf{c}_t}{\partial \mathbf{c}_{t-1}} \approx I$ et le gradient passe sans atténuation sur de longues séquences. C'est exactement le même mécanisme qu'une **connexion résiduelle** $\mathbf{z}_{\ell+1} = \mathbf{z}_\ell + f(\mathbf{z}_\ell)$, où le jacobien $I + \frac{\partial f}{\partial \mathbf{z}_\ell}$ contient un chemin identité direct.
```
