...
Cette connexion éclaire aussi le maximum de vraisemblance: si l'a priori est **uniforme** (ou constant), alors $\log p(\boldsymbol{\theta})$ est une constante qui n'affecte pas l'optimisation. Dans ce cas, MAP et MLE coïncident:

$$
\hat{\boldsymbol{\theta}}_{\text{MAP}} = \hat{\boldsymbol{\theta}}_{\text{MLE}} \quad \text{quand } p(\boldsymbol{\theta}) = \text{constante}
$$

Le MLE est donc un cas particulier du MAP: celui où nous n'avons aucune préférence a priori sur les paramètres. En d'autres termes, le MLE suppose implicitement que toutes les valeurs de paramètres sont également plausibles avant d'observer les données.
