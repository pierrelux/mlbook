...
La quantité $a = \boldsymbol{\theta}^\top \mathbf{x}$ avant la transformation sigmoïde s'appelle le **logit**. Elle a une interprétation naturelle: c'est le logarithme du **rapport de cotes** (log-odds):

$$
a = \log \frac{p(y = 1 | \mathbf{x})}{p(y = 0 | \mathbf{x})} = \log \frac{p(y = 1 | \mathbf{x})}{1 - p(y = 1 | \mathbf{x})}
$$

Si $a = 0$, les deux classes sont équiprobables. Si $a > 0$, la classe 1 est plus probable; si $a < 0$, c'est la classe 0. La valeur absolue de $a$ mesure la confiance: plus $|a|$ est grand, plus le modèle est certain de sa prédiction.
...
Comment trouver les paramètres $\boldsymbol{\theta}$? Comme pour la régression, nous utilisons le principe du maximum de vraisemblance: trouver les paramètres qui rendent les données observées les plus probables.

Sous l'hypothèse i.i.d., la vraisemblance est le produit des probabilités pour chaque exemple:
...
Contrairement à la régression linéaire qui admet une solution analytique, la régression logistique n'a pas de formule fermée pour $\boldsymbol{\theta}^*$. Nous devons recourir à l'optimisation numérique.
...
où $\eta > 0$ est le **taux d'apprentissage**. Le gradient indique la direction de plus forte croissance de la fonction; en allant dans la direction opposée, nous la diminuons.

Pour la régression logistique, la mise à jour est:
...
Calculer le gradient sur tout le jeu de données est coûteux pour $N$ grand. La **descente de gradient stochastique** (SGD) remplace le gradient exact par une estimation basée sur un sous-ensemble des données:

$$
\boldsymbol{\theta}_{t+1} = \boldsymbol{\theta}_t - \eta \cdot \frac{1}{|\mathcal{B}_t|} \sum_{i \in \mathcal{B}_t} (\mu_i - y_i) \mathbf{x}_i
$$

où $\mathcal{B}_t$ est un **mini-lot** (mini-batch) d'exemples échantillonnés aléatoirement.

L'estimation du gradient par mini-lot est **non biaisée**: son espérance égale le vrai gradient. La variance de l'estimation diminue avec la taille du mini-lot $|\mathcal{B}|$.

**Avantages du SGD**:
- Plus rapide par itération (sous-ensemble vs données complètes)
- Le bruit dans le gradient peut aider à s'échapper des minima locaux
- Permet le traitement de données en flux (streaming)
...
Le taux d'apprentissage $\eta$ est un hyperparamètre critique:
- **Trop petit**: convergence lente
- **Trop grand**: oscillations ou divergence

Une stratégie courante est de commencer avec un taux modéré et de le réduire au cours de l'entraînement. Par exemple, la décroissance en racine carrée $\eta_t = \eta_0 / \sqrt{t+1}$ garantit la convergence tout en permettant des pas initiaux plus grands.

## Implémentation

Voici une implémentation complète de la régression logistique binaire par SGD:
