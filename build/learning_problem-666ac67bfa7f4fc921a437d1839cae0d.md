...
Pour visualiser ce rétrécissement et comprendre son effet, examinons un exemple concret de régression linéaire avec deux caractéristiques. L'animation suivante montre simultanément trois perspectives sur la régularisation Ridge.
...
# Générer des données 2D avec colinéarité partielle
np.random.seed(42)
n = 25

# Caractéristique x1 et x2 corrélées (colinéarité)
x1 = np.random.randn(n)
x2 = 0.9 * x1 + 0.4 * np.random.randn(n)  # Forte corrélation avec x1

# Matrice de design (avec intercept absorbé pour simplifier)
...
theta2_range = np.linspace(-1, 3.5, 100)
...
# === Panneau 1: Espace des données ===
ax1 = fig.add_subplot(1, 3, 1)

# Projection des données sur la première composante principale pour visualisation
# On montre y vs la prédiction
y_pred_ols = X @ theta_ols
ax1.scatter(y_pred_ols, y, c='tab:blue', s=50, alpha=0.7, label='Données')

# Ligne de prédiction parfaite
pred_range = np.linspace(y.min() - 0.5, y.max() + 0.5, 100)
ax1.plot(pred_range, pred_range, 'k--', alpha=0.3, label='Prédiction parfaite')

# Point pour la prédiction Ridge (animé)
line_ridge_pred, = ax1.plot([], [], 'ro', markersize=8, alpha=0.7)
scatter_ridge = ax1.scatter([], [], c='tab:orange', s=50, alpha=0.7, label='Prédictions Ridge')

ax1.set_xlabel('Prédiction $\\hat{y}$')
ax1.set_ylabel('Valeur réelle $y$')
ax1.set_title('Prédictions vs réalité')
ax1.legend(loc='upper left', fontsize=9)
ax1.grid(True, alpha=0.3)
ax1.set_xlim(y.min() - 1, y.max() + 1)
ax1.set_ylim(y.min() - 1, y.max() + 1)
ax1.set_aspect('equal')

# Texte pour MSE
mse_text = ax1.text(0.98, 0.02, '', transform=ax1.transAxes, fontsize=10, 
                    ha='right', va='bottom',
...
ax2.annotate('$\\hat{\\boldsymbol{\\theta}}_{\\text{MCO}}$', 
             xy=(theta_ols[0], theta_ols[1]), 
             xytext=(theta_ols[0] + 0.3, theta_ols[1] + 0.3),
             fontsize=11, ha='left')

# Origine
ax2.plot(0, 0, 'k+', markersize=15, markeredgewidth=2, zorder=4)
ax2.annotate('Origine', xy=(0, 0), xytext=(0.2, -0.4), fontsize=10, color='gray')

# Cercle de contrainte Ridge (animé) - représente ||theta||^2 = c
...
ax2.plot(theta_path[:, 0], theta_path[:, 1], 'tab:orange', linewidth=1.5, 
         alpha=0.4, linestyle='--', label='Chemin Ridge')

ax2.set_xlabel('$\\theta_1$')
ax2.set_ylabel('$\\theta_2$')
ax2.set_title('Paysage de perte et contrainte Ridge')
...
ax3.plot(lambda_range, shrink1_curve, 'b-', linewidth=2, 
         label=f'Direction 1 ($d_1={d_svd[0]:.2f}$)')
ax3.plot(lambda_range, shrink2_curve, 'r-', linewidth=2, 
         label=f'Direction 2 ($d_2={d_svd[1]:.2f}$)')
...
ax3.set_ylabel('Facteur $\\frac{d_j^2}{d_j^2 + \\lambda}$')
ax3.set_title('Rétrécissement par direction')
ax3.legend(loc='upper right', fontsize=9)
ax3.grid(True, alpha=0.3)
ax3.set_xlim(0, 10)
ax3.set_ylim(0, 1.1)

shrink_text = ax3.text(0.98, 0.5, '', transform=ax3.transAxes, fontsize=10, 
                       va='center', ha='right',
                       bbox=dict(boxstyle='round', facecolor='white', alpha=0.8))
...
def animate(frame):
    # Lambda augmente de façon exponentielle pour mieux voir les petites valeurs
    if frame < 80:
        lam = (frame / 80) * 10
    else:
        lam = 10  # Pause à la fin
    
    # Solution Ridge
    theta_ridge = ridge_solution(X, y, lam)
    
    # Panneau 1: Mettre à jour les prédictions
    y_pred_ridge = X @ theta_ridge
    scatter_ridge.set_offsets(np.column_stack([y_pred_ridge, y]))
    mse_ols = np.mean((y - X @ theta_ols)**2)
    mse_ridge = np.mean((y - y_pred_ridge)**2)
    mse_text.set_text(f'MSE MCO: {mse_ols:.3f}\nMSE Ridge: {mse_ridge:.3f}')
    ...
    lambda_text.set_text(f'$\\lambda = {lam:.2f}$\n$\\|\\boldsymbol{{\\theta}}\\|_2 = {norm_ridge:.2f}$')
    ...
    shrink_text.set_text(f'Dir. 1: {shrink1:.2f}\nDir. 2: {shrink2:.2f}')
    
    return (scatter_ridge, point_ridge, circle_ridge, lambda_text, 
            point_shrink1, point_shrink2, mse_text, shrink_text)
...
**Panneau de gauche — Prédictions**: Les points bleus montrent les prédictions MCO ($\lambda = 0$), les points orange les prédictions Ridge. À mesure que $\lambda$ augmente, les prédictions Ridge se rapprochent de la moyenne (variance réduite), mais s'éloignent légèrement de la ligne de prédiction parfaite (biais introduit). Le MSE Ridge augmente sur les données d'entraînement, mais sur de nouvelles données, il serait souvent meilleur grâce à la réduction de variance.

**Panneau central — Paysage de perte**: Les contours gris représentent la fonction de coût RSS (somme des carrés des résidus). La solution MCO (point noir) est au centre des ellipses. Le cercle orange représente la contrainte $\|\boldsymbol{\theta}\|_2 = c$ imposée par Ridge: plus $\lambda$ est grand, plus le cercle est petit, forçant la solution à se rapprocher de l'origine. La solution Ridge (point orange) suit le **chemin de régularisation** (ligne pointillée), se déplaçant de MCO vers l'origine.

**Panneau de droite — Rétrécissement différencié**: Les deux courbes montrent les facteurs de rétrécissement pour chaque direction principale. La direction 2 (faible valeur singulière $d_2$, correspondant à la direction de forte colinéarité) est rétrécit beaucoup plus rapidement que la direction 1. C'est le cœur de l'effet Ridge: **pénaliser davantage les directions où le signal est faible** (et donc où l'estimation est instable).

L'intuition géométrique est la suivante: dans les directions où les données sont dispersées (grandes valeurs singulières), la RSS varie rapidement et la solution est bien déterminée. Dans les directions où les données sont concentrées (petites valeurs singulières, colinéarité), la RSS varie lentement (ellipse allongée) et de petites perturbations causent de grands déplacements de la solution. La régularisation contraint ces directions instables en premier.
