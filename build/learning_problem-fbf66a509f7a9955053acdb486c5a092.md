...
Cette formule est simple à calculer: nous parcourons nos $N$ exemples d'entraînement, calculons la perte pour chacun, et faisons la moyenne. C'est exactement ce que fait le code suivant:

```python
def empirical_risk(f, dataset, loss_function):
    total_loss = 0
    for x, y in dataset:
        prediction = f(x)
