...
# Fonction de régression (même pour les deux cas)
x_line = np.linspace(0, 10, 100)
f_mu = lambda x: 0.5 * x + 1  # moyenne: fonction linéaire

# Points x où afficher les gaussiennes
x_points = [2, 4, 6, 8]
y_range = np.linspace(-2, 8, 200)

# Échelle pour afficher les gaussiennes horizontalement
scale = 1.5

# --- Panneau gauche: Homoscédastique ---
ax = axes[0]
sigma_homo = 0.8  # variance constante

# Tracer la ligne de régression
ax.plot(x_line, f_mu(x_line), 'k-', linewidth=2, label=r'$f_\mu(\mathbf{x})$')

# Tracer les gaussiennes à chaque point
for x_pt in x_points:
    mu = f_mu(x_pt)
    pdf = norm.pdf(y_range, mu, sigma_homo)
    # Afficher la gaussienne "horizontalement" (pdf sur l'axe x)
    ax.fill_betweenx(y_range, x_pt, x_pt + scale * pdf, alpha=0.3, color='steelblue')
    ax.plot(x_pt + scale * pdf, y_range, color='steelblue', linewidth=1)
    # Ligne verticale au centre
    ax.plot([x_pt, x_pt], [mu - 2*sigma_homo, mu + 2*sigma_homo], 
            'steelblue', linewidth=0.5, linestyle='--', alpha=0.5)

ax.set_xlim(-0.5, 12)
ax.set_ylim(-1, 7)
ax.set_xlabel(r'$x$')
ax.set_ylabel(r'$y$')
ax.set_title('Régression homoscédastique\n' + r'$\sigma^2$ constant')
ax.set_aspect('equal')

# --- Panneau droit: Hétéroscédastique ---
ax = axes[1]
f_sigma = lambda x: 0.3 + 0.15 * x  # variance croissante avec x

# Tracer la ligne de régression
ax.plot(x_line, f_mu(x_line), 'k-', linewidth=2, label=r'$f_\mu(\mathbf{x})$')

# Tracer les gaussiennes à chaque point (variance variable)
for x_pt in x_points:
    mu = f_mu(x_pt)
    sigma = f_sigma(x_pt)
    pdf = norm.pdf(y_range, mu, sigma)
    ax.fill_betweenx(y_range, x_pt, x_pt + scale * pdf, alpha=0.3, color='coral')
    ax.plot(x_pt + scale * pdf, y_range, color='coral', linewidth=1)
    ax.plot([x_pt, x_pt], [mu - 2*sigma, mu + 2*sigma], 
            'coral', linewidth=0.5, linestyle='--', alpha=0.5)

ax.set_xlim(-0.5, 12)
ax.set_ylim(-1, 7)
ax.set_xlabel(r'$x$')
ax.set_ylabel(r'$y$')
ax.set_title('Régression hétéroscédastique\n' + r'$\sigma^2(x)$ variable')
ax.set_aspect('equal')

plt.tight_layout()
plt.savefig('_static/homoscedastic_heteroscedastic.svg', bbox_inches='tight')
plt.show()
```

La figure illustre la différence entre les deux modèles. À chaque valeur de $x$, la distribution conditionnelle $p(y|x)$ est une gaussienne centrée sur la courbe de régression $f_\mu(x)$. Dans le cas homoscédastique (gauche), toutes les gaussiennes ont la même largeur. Dans le cas hétéroscédastique (droite), la largeur varie — ici, l'incertitude augmente avec $x$.
