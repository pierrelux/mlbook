...
# Dessiner les axes
ax2.arrow(0, 0, 3, 0, head_width=0.1, head_length=0.1, 
          fc='gray', ec='gray', alpha=0.5, linewidth=1.5)
ax2.arrow(0, 0, 0, 3, head_width=0.1, head_length=0.1, 
          fc='gray', ec='gray', alpha=0.5, linewidth=1.5)
ax2.text(3.2, 0, '$\\mathbf{v}_1$ (direction $d_1={:.1f}$)'.format(d_svd[0]), 
         fontsize=11, ha='left', color='gray', va='center')
ax2.text(0, 3.2, '$\\mathbf{v}_2$ (direction $d_2={:.1f}$)'.format(d_svd[1]), 
         fontsize=11, ha='center', color='gray')

# Solution OLS dans l'espace V
ax2.plot(theta_ols_v[0], theta_ols_v[1], 'ko', markersize=10, label='MCO', zorder=5)
ax2.text(theta_ols_v[0]+0.15, theta_ols_v[1]+0.15, 
...
for lam, color in zip(lambdas[1:], colors[1:]):
    ...
    # Flèche montrant le rétrécissement
    ax2.arrow(theta_ols_v[0], theta_ols_v[1], 
             theta_ridge_v[0] - theta_ols_v[0], theta_ridge_v[1] - theta_ols_v[1],
             head_width=0.1, head_length=0.1, fc=color, ec=color, 
             alpha=0.6, linewidth=2, linestyle='--')
    
    # Annoter le facteur de rétrécissement
    mid_x = (theta_ols_v[0] + theta_ridge_v[0]) / 2
    mid_y = (theta_ols_v[1] + theta_ridge_v[1]) / 2
    ax2.text(mid_x, mid_y, f'$\\frac{{d_j^2}}{{d_j^2+{lam}}}$', 
