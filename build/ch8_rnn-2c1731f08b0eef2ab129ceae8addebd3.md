...
- Expliquer le problème du gradient qui disparaît et les mécanismes de portes (LSTM, GRU)
```
...
Le LSTM (*Long Short-Term Memory*) {cite}`hochreiter1997long` résout le problème du gradient qui disparaît en introduisant un état de cellule $\mathbf{c}_t$ qui circule d'un pas à l'autre de façon (presque) linéaire. Des portes apprises contrôlent le flux d'information:
...
Ce qui compte pour l'intuition, c'est le mécanisme commun: les portes permettent au gradient de circuler sans être multiplié par $W_{hh}$ à chaque pas, ce qui atténue le problème du gradient qui disparaît.
...
Le produit de jacobiennes qui apparaît dans BPTT décroît (ou croît) exponentiellement avec le nombre de pas de temps. Ce phénomène de gradient qui disparaît empêche l'apprentissage de dépendances à long terme. Le LSTM et le GRU introduisent des portes qui créent un chemin linéaire pour le gradient, atténuant ce problème.
...
3. La rétropropagation à travers le temps (BPTT) applique la règle de la chaîne au réseau déroulé. Le gradient implique un produit de jacobiennes qui décroît exponentiellement: c'est le problème du gradient qui disparaît.
...
2. Pour $t > 3$, $h_t = 0{,}5 \cdot h_{t-1}$. L'état décroît géométriquement vers 0: $h_4 = -0{,}375$, $h_5 = -0{,}1875$, etc. Puisque $|w_{hh}| = 0{,}5 < 1$, le réseau "oublie" progressivement. C'est une manifestation du gradient qui disparaît dans un cas linéaire simplifié.
````

````{admonition} Exercice 3: Gradient qui disparaît ★★
:class: hint dropdown
...
Si $|w_{hh}| < 1$, ce terme décroît exponentiellement vers 0: le gradient disparaît et le réseau ne peut pas apprendre les dépendances entre $h_k$ et la perte au temps $t$. Si $|w_{hh}| > 1$, le terme croît exponentiellement: le gradient explose. Le cas multidimensionnel est analogue, avec les valeurs propres de $W_{hh}$ jouant le rôle de $w_{hh}$.
````
