...
La section précédente a décrit la structure du décodeur et le masque causal, mais elle n'a pas détaillé comment on entraîne un modèle de langage ni comment on génère du texte avec. Cette section rend ces deux procédures explicites et les compare à leur équivalent pour les réseaux récurrents.
...
1. Pour $t = 1, 2, \ldots$:
   - Construire la matrice d'entrée $X \in \mathbb{R}^{t \times d}$ à partir de $(x_1, \ldots, x_t)$
   - Pour chaque bloc $\ell = 1, \ldots, N$:
     - $X \leftarrow \text{BlocTransformeur}_\ell(X)$ avec masque causal
   - $p_t = \text{softmax}(W_{\text{sortie}}\, \mathbf{x}_t^{(N)} + \mathbf{b})$ où $\mathbf{x}_t^{(N)}$ est la dernière ligne de $X$
   - Échantillonner $x_{t+1} \sim \text{Catégorielle}(p_t)$
   - Si $x_{t+1} = \langle\text{fin}\rangle$, arrêter
2. Retourner $(x_1, x_2, \ldots)$
```
