...
2. **Rétrécissement des coefficients** (*shrinkage*): Pour comprendre cet effet, utilisons la **décomposition en valeurs singulières** (SVD) de $\mathbf{X} = \mathbf{U} \mathbf{D} \mathbf{V}^\top$, où $\mathbf{U}$ et $\mathbf{V}$ contiennent des directions orthonormales et $\mathbf{D}$ est diagonale avec les valeurs singulières $d_1 \geq d_2 \geq \cdots \geq d_d \geq 0$. La solution MCO peut s'écrire:
