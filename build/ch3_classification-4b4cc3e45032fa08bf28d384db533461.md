...
- Appliquer le modèle de Bradley-Terry pour l'apprentissage de préférences
...
## Modèle de Bradley-Terry pour les préférences

La régression logistique trouve une application élégante dans la modélisation des **préférences**. Plutôt que de classifier des exemples dans des catégories, nous voulons prédire lequel de deux objets sera préféré lors d'une comparaison.

### Formulation du modèle

Imaginons un tournoi d'échecs où nous observons les résultats de matchs entre joueurs. Le **modèle de Bradley-Terry** (1952) suppose que chaque joueur $i$ possède une «force» latente $s_i \in \mathbb{R}$. La probabilité que le joueur $i$ batte le joueur $j$ est:

$$
P(i \succ j) = \sigma(s_i - s_j) = \frac{1}{1 + e^{-(s_i - s_j)}}
$$

C'est exactement une régression logistique où l'entrée est la différence de forces $s_i - s_j$ et la sortie est binaire (victoire ou défaite). Un joueur avec un score plus élevé a plus de chances de gagner, mais des upsets restent possibles.

```{margin} Le système Elo aux échecs
Le classement Elo, utilisé aux échecs depuis les années 1960, est une variante du modèle de Bradley-Terry. La différence de scores Elo prédit la probabilité de victoire: une différence de 400 points correspond à une probabilité d'environ 91% pour le joueur le plus fort.
```

### Estimation des forces

Supposons que nous observons $N$ matchs $\{(i_n, j_n, y_n)\}_{n=1}^N$ où $y_n = 1$ si le joueur $i_n$ gagne et $y_n = 0$ sinon. La log-vraisemblance est:

$$
\log \mathcal{L}(\mathbf{s}) = \sum_{n=1}^N \left[ y_n \log \sigma(s_{i_n} - s_{j_n}) + (1-y_n) \log \sigma(s_{j_n} - s_{i_n}) \right]
$$

C'est l'entropie croisée binaire avec les différences de scores comme prédicteurs. Les forces $\mathbf{s}$ s'estiment par maximum de vraisemblance, comme pour la régression logistique.
...
# Simulation: 4 joueurs avec forces vraies
np.random.seed(42)
true_strengths = np.array([2.0, 1.0, 0.0, -1.0])  # Joueur 0 est le plus fort
...
for _ in range(n_matches):
    ...
    outcome = np.random.random() < prob_i_wins
    matches.append((i, j, int(outcome)))

# Estimation par descente de gradient
def bradley_terry_loss(s, matches):
    loss = 0
    for i, j, y in matches:
        logit = s[i] - s[j]
        loss -= y * np.log(1/(1+np.exp(-logit)) + 1e-10)
        loss -= (1-y) * np.log(1/(1+np.exp(logit)) + 1e-10)
    return loss / len(matches)

def bradley_terry_grad(s, matches):
    grad = np.zeros_like(s)
    for i, j, y in matches:
        prob = 1 / (1 + np.exp(-(s[i] - s[j])))
        grad[i] += (prob - y)
        grad[j] -= (prob - y)
    return grad / len(matches)

# Optimisation
s_est = np.zeros(n_players)
lr = 0.5
for _ in range(100):
    s_est -= lr * bradley_terry_grad(s_est, matches)
# Centrer (les forces sont définies à une constante près)
s_est -= s_est.mean()
...
width = 0.35
bars1 = ax.bar(x - width/2, true_strengths - true_strengths.mean(), width, 
               label='Forces vraies', color='steelblue', alpha=0.7)
bars2 = ax.bar(x + width/2, s_est, width, 
               label='Forces estimées', color='coral', alpha=0.7)

ax.set_xlabel('Joueur')
ax.set_ylabel('Force (centrée)')
ax.set_title(f'Estimation Bradley-Terry ({n_matches} matchs)')
ax.set_xticks(x)
...
La figure montre que les forces estimées à partir des résultats de matchs correspondent bien aux forces vraies. L'estimation est définie à une constante additive près (nous centrons les scores).

### Applications modernes

Le modèle de Bradley-Terry est omniprésent dans les systèmes de classement et d'évaluation:

- **Classements sportifs**: systèmes Elo (échecs), Glicko (jeux en ligne), TrueSkill (Xbox)
- **Moteurs de recherche**: apprendre à ordonner des résultats selon les clics des utilisateurs
- **Systèmes de recommandation**: modéliser les préférences implicites

Plus récemment, ce modèle joue un rôle central dans l'**alignement des grands modèles de langage**. Lors du *fine-tuning* par renforcement (RLHF), des annotateurs humains comparent des paires de réponses générées par le modèle et indiquent leur préférence. Ces préférences alimentent un modèle de Bradley-Terry qui apprend une fonction de **récompense**: le score $s$ d'une réponse prédit sa qualité. Cette récompense guide ensuite l'optimisation du modèle de langage.

```{admonition} De la préférence à la récompense
:class: note

Si nous observons que la réponse $A$ est préférée à $B$ avec probabilité $P(A \succ B) = \sigma(r_A - r_B)$, le score $r$ peut être interprété comme une récompense. La différence $r_A - r_B$ mesure à quel point $A$ est meilleur que $B$. C'est ce principe qui transforme des jugements humains qualitatifs en un signal quantitatif pour l'apprentissage par renforcement.
```
...
- Le **modèle de Bradley-Terry** applique la régression logistique aux comparaisons par paires: $P(i \succ j) = \sigma(s_i - s_j)$. Il est central dans les systèmes de classement (Elo) et l'alignement des grands modèles de langage (RLHF).
