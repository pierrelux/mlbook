...
### Arbres k-d: l'idée en une phrase
...
C'est tout. Le reste n'est que détails d'implémentation.

### Le principe: élaguer par boîtes englobantes
...
### Comment l'arbre découpe l'espace

L'arbre k-d partitionne récursivement l'espace. À chaque nœud, on choisit une dimension et on divise les points en deux groupes selon leur coordonnée sur cette dimension (typiquement au niveau de la médiane). On alterne les dimensions à chaque niveau.
...
ax.set_ylabel('$x_2$')
ax.set_aspect('equal')
ax.set_title('Partition de l\'espace par un arbre k-d')
...
### En pratique: utilisez une bibliothèque

Vous n'avez pas à implémenter un arbre k-d. Les bibliothèques le font pour vous:
...
L'option `algorithm='auto'` choisit la meilleure stratégie selon la taille et la dimension des données. C'est généralement le bon choix.

```{admonition} Ce qu'il faut retenir
:class: note

1. **L'idée**: éviter de calculer des distances inutiles en éliminant des régions entières
2. **Le test**: si la distance minimale à une boîte dépasse le meilleur candidat, élaguer
3. **La limite**: efficace seulement en basse dimension ($d \lesssim 20$)
4. **En pratique**: utiliser `algorithm='auto'` dans scikit-learn
```
