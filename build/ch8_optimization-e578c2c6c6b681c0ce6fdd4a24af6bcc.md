...
titles = ['SGD', r'SGD + Momentum ($\beta=0{,}9$)']
...
Sur cette surface allongée, SGD progresse lentement: chaque pas est petit et le gradient oscille entre les parois de la vallée. Avec le momentum ($\beta = 0{,}9$), la dynamique est qualitativement différente. La vitesse accumulée propulse les paramètres le long de la vallée, mais l'inertie les fait dépasser le minimum et osciller de l'autre côté, comme une bille qui roule dans un bol. Ces oscillations s'amortissent au fil des itérations et la convergence est globalement plus rapide que SGD, malgré les dépassements.
...
Dans un réseau de $L$ couches avec activation sigmoïde, chaque couche $\ell$ contribue un facteur $\frac{\partial \mathbf{z}_\ell}{\partial \mathbf{z}_{\ell-1}} = \text{diag}(\sigma'(\mathbf{a}_\ell))\, W_\ell$ à la chaîne de jacobiennes. La norme de chaque facteur est bornée par $\|\sigma'(\mathbf{a}_\ell)\|_\infty \|W_\ell\|$. Même avec des poids bien calibrés, le facteur $\sigma'$ plafonne à 0,25. Après $L$ couches, la norme du gradient est au plus $0{,}25^L$ fois sa valeur initiale. Pour $L = 20$, cela donne $(0{,}25)^{20} \approx 10^{-12}$: le gradient est pratiquement nul.
...
L'équivalence entre régularisation L2 et décroissance des poids est exacte pour SGD, mais pas pour les optimiseurs adaptatifs comme Adam. Avec Adam, ajouter $\lambda\boldsymbol{\theta}$ au gradient modifie aussi les statistiques des moments, ce qui change le comportement de la régularisation. **AdamW** {cite}`loshchilov2019decoupled` corrige cela en appliquant la décroissance des poids directement sur les paramètres, séparément de la mise à jour du gradient: $\boldsymbol{\theta}_{t+1} = (1 - \eta\lambda)\boldsymbol{\theta}_t - \eta\, \hat{\mathbf{m}} / (\sqrt{\hat{\mathbf{s}}} + \epsilon)$. AdamW est devenu le choix standard pour l'entraînement de grands modèles.
