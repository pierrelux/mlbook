...
Un problème survient quand une combinaison caractéristique-classe n'apparaît jamais dans les données d'entraînement. Si le mot «bitcoin» n'apparaît dans aucun courriel légitime, nous avons $\hat{\theta}_{dc} = 0$. Lors de la classification d'un nouveau courriel contenant «bitcoin», le produit $\prod_d p(x_d \mid y = \text{légitime})$ devient nul, quelle que soit la valeur des autres caractéristiques. Un seul mot peut ainsi dominer entièrement la décision.
...
# Scénario: 10 pourriels, 0 avec le mot "bitcoin"
N_spam = 10
N_bitcoin_spam = 0
K = 2  # binaire

# EMV
theta_mle = N_bitcoin_spam / N_spam

# Avec lissage de Laplace
alphas = [0, 0.1, 0.5, 1, 2, 5]
thetas = [(N_bitcoin_spam + alpha) / (N_spam + K * alpha) for alpha in alphas]
...
ax.set_title('Effet du lissage sur $p(\\text{«bitcoin»} \\mid \\text{pourriel})$\n(0 occurrence sur 10 exemples)')
...
ax.set_xlabel('$x_1$')
ax.set_ylabel('$x_2$')
ax.set_title('Partitionnement dur (cluster le plus probable)')
ax.legend()
ax.set_xlim(-3, 7)
ax.set_ylim(-3, 6)
...
4. **Avantages de EM sur k-moyennes:**
   ...
   - **Formes de clusters:** GMM capture des ellipses, k-moyennes ne fait que des sphères
...
3. Générez 500 points et visualisez-les. Les clusters sont-ils visibles?
...
3. **Visualisation:** Les trois clusters devraient être visibles, avec le cluster 2 plus étalé (variance 2) et le cluster 3 légèrement allongé (corrélation positive).
