...
3. **Lien avec l'entropie croisée:**
   ...
   $$
   \text{BCE}(\boldsymbol{\theta}) = -\frac{1}{N} \sum_{i=1}^N \left[ y_i \log \sigma_i + (1-y_i) \log(1 - \sigma_i) \right]
   $$
   
   On voit que $\text{BCE}(\boldsymbol{\theta}) = -\frac{1}{N} \log \mathcal{L}(\boldsymbol{\theta})$.
   
   Donc: $\max_{\boldsymbol{\theta}} \log \mathcal{L}(\boldsymbol{\theta}) \Leftrightarrow \min_{\boldsymbol{\theta}} \text{BCE}(\boldsymbol{\theta})$
...
````{admonition} Exercice 8: MLE comme cas particulier de MAP ★
:class: hint dropdown
...
1. Écrivez la log-posterior $\log p(\boldsymbol{\theta} | \mathcal{D})$ en fonction de la log-vraisemblance et du log-prior.

2. Supposons un prior uniforme (constant): $p(\boldsymbol{\theta}) = c$ pour tout $\boldsymbol{\theta}$. Montrez que l'estimateur MAP se réduit à l'estimateur du maximum de vraisemblance (EMV).

3. Pour quels autres types de priors l'EMV et le MAP coïncident-ils?

4. Expliquez pourquoi utiliser un prior uniforme peut être problématique dans certains cas.
````
...
2. **Prior uniforme:**
   ...
   L'EMV est donc un cas particulier du MAP avec prior uniforme.

3. **Autres priors:**

   L'EMV et le MAP coïncident pour tout prior qui est constant sur le domaine des paramètres, ou plus généralement, pour tout prior dont le log est constant (à une constante additive près). Cela inclut les priors impropres (non normalisables) qui sont uniformes sur $\mathbb{R}^d$.

4. **Problèmes du prior uniforme:**

   - **Avec peu de données**: l'EMV peut être extrême. Exemple: 3 lancers de pièce donnant 3 faces → EMV = 100% de probabilité de face.
   - **Paramètres non bornés**: un prior uniforme sur $\mathbb{R}$ n'est pas une vraie distribution de probabilité (prior impropre).
   - **Invariance**: un prior uniforme sur $\theta$ n'est pas uniforme sur $g(\theta)$ pour une transformation non-linéaire $g$.
...
4. **Entropie croisée:**
   ...
   $$
   \text{BCE} = -\sum_{c=1}^C y_c \log \hat{p}_c = -(0 \cdot \log 0.1 + 1 \cdot \log 0.7 + 0 \cdot \log 0.2)
   $$
...
````{admonition} Exercice 11: MAP avec prior gaussien et régression ridge ★★
:class: hint dropdown
...
avec un prior gaussien isotrope sur les paramètres:
...
2. Écrivez le log-prior $\log p(\boldsymbol{\theta})$.
...
4. **Interprétation:**
   ...
   - **Grand $\tau^2$** (prior large): on est peu sûr que les paramètres sont proches de zéro, donc $\lambda$ petit, peu de régularisation, MAP proche de EMV.
   - **Petit $\tau^2$** (prior concentré): on croit fortement que les paramètres sont proches de zéro, donc $\lambda$ grand, forte régularisation, coefficients tirés vers zéro.
   
   Le rapport $\sigma^2/\tau^2$ compare l'incertitude dans les données ($\sigma^2$) à l'incertitude dans le prior ($\tau^2$). Plus les données sont bruitées, plus on fait confiance au prior.
```
...
5. Tracez les distributions prior, posterior après 1 observation, et posterior après 2 observations. Que remarquez-vous sur l'évolution de l'incertitude?
````

```{admonition} Solution Exercice 18
:class: dropdown

1. **Calcul de la posterior:**

   Le posterior est proportionnel à:
   $$
   ...
   C'est une forme quadratique en $\theta$, donc le posterior est gaussien.

2. **Paramètres du posterior:**
...
3. **Estimateur MAP:**
   ...
   Pour une posterior gaussienne, le mode = la moyenne:
   $$
...
4. **Mise à jour séquentielle:**
   ...
   On utilise le posterior après la première observation comme nouveau prior:
...
5. **Visualisation:**
   ...
   plt.plot(theta, post1, label='Posterior (1 obs)')
   plt.plot(theta, post2, label='Posterior (2 obs)')
   plt.legend()
   ```
   
   Observations:
   - Le prior est large (grande incertitude)
   - Après 1 observation, le posterior se concentre autour de 1.2
   - Après 2 observations, le posterior se concentre davantage (variance diminue)
   - La moyenne a posteriori est une moyenne pondérée du prior et des données
```
