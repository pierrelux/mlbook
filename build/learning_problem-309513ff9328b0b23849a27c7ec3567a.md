...
La complexité de Monte Carlo est **indépendante de la dimension** $d$. Elle ne dépend que du nombre d'échantillons $N$. C'est cette propriété qui rend l'apprentissage possible en haute dimension. De plus, nous n'avons même pas besoin de connaître la valeur numérique de $p(\mathbf{x},y)$; il suffit de pouvoir tirer des échantillons de cette distribution. C'est exactement ce que nos données d'entraînement nous fournissent.
...
Le second terme ne dépend pas de $\boldsymbol{\theta}$. Minimiser la NLL revient donc exactement à minimiser la somme des erreurs quadratiques.
...
Avant d'introduire l'interprétation bayésienne, illustrons pourquoi le maximum de vraisemblance peut être problématique. Considérons un exemple simple: estimer la probabilité $\theta$ qu'une pièce tombe sur face.
...
Le problème est que le MLE dispose de suffisamment de flexibilité pour reproduire parfaitement les données d'entraînement, même quand celles-ci sont peu nombreuses ou non représentatives. C'est un problème de **généralisation**: le modèle est trop optimiste sur sa capacité à prédire.
...
La **statistique bayésienne** offre une solution élégante: au lieu d'estimer un point unique pour les paramètres, elle caractérise notre **incertitude** sur ces paramètres. Cette incertitude est représentée par une distribution de probabilité.
...
Cette estimation est plus raisonnable que $\theta_{\text{MLE}} = 1$. L'a priori "tire" l'estimation vers des valeurs moins extrêmes.
...
La figure de gauche montre comment l'a posteriori combine l'a priori et la vraisemblance. L'a priori Beta(2,2) "tire" l'estimation vers 0.5, résultant en un MAP de 0.8 au lieu du MLE de 1.0. La figure de droite montre l'effet de différents a priori: plus l'a priori est fort (variance faible), plus l'estimation est proche de 0.5.
