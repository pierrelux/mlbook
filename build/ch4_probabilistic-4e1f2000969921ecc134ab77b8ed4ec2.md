...
L'EMV peut donner des résultats extrêmes avec peu de données. Considérons l'exemple suivant.
...
$$
\log \mathcal{L}(\theta) = 3 \log \theta
$$

$$
\frac{d}{d\theta} \log \mathcal{L}(\theta) = \frac{3}{\theta} = 0
$$

Cette dérivée n'a pas de zéro dans $(0, 1)$, mais elle est toujours positive: la log-vraisemblance croît avec $\theta$. Le maximum est donc atteint à la borne $\theta = 1$.

Résultat: $\hat{\theta}_{\text{EMV}} = 1$. L'EMV prédit que la pièce tombe toujours sur face!

Dans le cas général avec $k$ faces sur $N$ lancers:

$$
\frac{d}{d\theta} \log \mathcal{L}(\theta) = \frac{k}{\theta} - \frac{N - k}{1 - \theta} = 0
$$

En résolvant: $\hat{\theta}_{\text{EMV}} = k / N$, la fréquence empirique des faces.
```

Cet exemple illustre une limitation de l'EMV: avec peu d'observations, l'estimateur peut être extrême. Le MAP permet de modérer ces estimations en incorporant une connaissance a priori.
...
Comparons avec le risque empirique régularisé que nous avons vu au chapitre sur la [généralisation](ch3_generalization.md):

$$
\hat{\boldsymbol{\theta}} = \arg\min_{\boldsymbol{\theta}} \left[ \frac{1}{N} \sum_{i=1}^N \ell(y_i, f(\mathbf{x}_i; \boldsymbol{\theta})) + \lambda \, \Omega(\boldsymbol{\theta}) \right]
$$
...
La régularisation correspond donc à l'ajout d'un a priori sur les paramètres. Le terme $R(\boldsymbol{\theta}) = -\log p(\boldsymbol{\theta})$ joue le rôle du régulariseur $\lambda \, \Omega(\boldsymbol{\theta})$.
