...
où $r_{nk} \in \{0, 1\}$ est l'assignation du point $n$ au groupe $k$ (avec $\sum_k r_{nk} = 1$). Minimiser cette fonction simultanément sur les assignations et les centroïdes est un problème combinatoire. K-moyennes le résout en alternant deux étapes:
...
# K-moyennes: convergence sur des données 2D
np.random.seed(42)
n_samples = 300
X_kmeans = np.vstack([
...
iters_show = [0, min(2, len(hist_km)-1), len(hist_km)-1]

for ax, it in zip(axes, iters_show):
    mu_t, labels_t = hist_km[it]
    for k in range(3):
        mask = labels_t == k
        ax.scatter(X_kmeans[mask, 0], X_kmeans[mask, 1], c=colors_km[k], alpha=0.4, s=15)
...
fig.suptitle('K-moyennes: les croix marquent les centroïdes', fontsize=12, y=1.02)
plt.tight_layout()
```
