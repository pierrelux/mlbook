...
# Surface: f(theta) = theta1^2 + 10*theta2^2  (vallée allongée)
...
for ax, traj, title, color in zip(axes, trajs, titles, colors):
    ...
    ax.plot(*theta0, 'ko', ms=8, zorder=5, label='Départ')
    ...
    ax.set_ylim(-1.0, 1.0)
    ax.grid(True, alpha=0.2)
    # Segments de trajectoire (un par pas, initialement vides)
    segs = [ax.plot([], [], '-', color=color, lw=1.8)[0] for _ in range(n_steps)]
    line_segments.append(segs)
    dot, = ax.plot([], [], 'o', color=color, ms=7, zorder=6)
    dots_current.append(dot)
...
def init():
    for segs in line_segments:
        for seg in segs:
            seg.set_data([], [])
    for dot in dots_current:
        dot.set_data([], [])
    return [seg for segs in line_segments for seg in segs] + dots_current

def animate(frame):
    k = frame  # étape courante (0 .. n_steps-1)
    for segs, traj, dot in zip(line_segments, trajs, dots_current):
        # Afficher tous les segments jusqu'à k
        for i, seg in enumerate(segs):
            if i < k:
                alpha = 0.3 + 0.7 * (i / n_steps)
                seg.set_data(traj[i:i+2, 0], traj[i:i+2, 1])
                seg.set_alpha(alpha)
            else:
                seg.set_data([], [])
        # Point courant
        dot.set_data([traj[k, 0]], [traj[k, 1]])
    return [seg for segs in line_segments for seg in segs] + dots_current

anim = animation.FuncAnimation(
    fig, animate, init_func=init,
    frames=n_steps + 1, interval=120, blit=True
)
plt.close(fig)
HTML(anim.to_jshtml())
```
...
Les numéros indiquent l'ordre de la passe arrière: on commence par $f$ (noeud ①), puis on descend. Le noeud $x$ (⑤) doit attendre que $v_1$ (③) *et* $v_2$ (④) aient tous deux envoyé leur gradient.
...
Contrairement au DAG, la tape est une **liste ordonnée** (un tableau linéaire), ce qui la rend simple à parcourir dans les deux sens. Les flèches pleines montrent l'enregistrement (passe avant); les flèches pointillées montrent le rejeu (passe arrière).
...
La tape démarre vide et grandit à chaque opération (flèches pleines, gauche à droite). Une fois $f$ calculée, la passe arrière initialise $\bar{v}_3 = 1$ puis remonte la liste à rebours (flèches pointillées, droite à gauche).
...
Chaque ligne de la tape correspond à une opération élémentaire. La passe arrière part de l'adjoint $\bar{v}_3 = 1$ (gradient de $f$ par rapport à lui-même) et remonte: l'étape 3 envoie des gradients à $v_1$ et $v_2$, puis les étapes 2 et 1 envoient leurs gradients à $x$ et $y$.

### Le traceur

Comment une bibliothèque comme JAX construit-elle cette tape automatiquement, sans modifier le programme utilisateur? La réponse est le **traceur** (*tracer*).
...
2. **Enregistre l'opération sur le tape**, avec ses entrées et une fermeture (*closure*) qui sait calculer les gradients locaux,
...
graph TD
    ...
    v1["<b>v₁ = sin(x)</b><br/>data = 0.479<br/>grad = 0.0<br/><i>backward: x.grad += cos(x)·v̄₁</i>"]
    v2["<b>v₂ = x + y</b><br/>data = 1.7<br/>grad = 0.0<br/><i>backward: x.grad += v̄₂, y.grad += v̄₂</i>"]
    v3["<b>v₃ = v₁ · v₂</b><br/>data = 0.814<br/><b>grad = 1.0</b><br/><i>backward: v₁.grad += v₂·v̄₃, v₂.grad += v₁·v̄₃</i>"]
...
ax.text(5, 1.2, 'Construit le tape\nautomatiquement', ha='center', fontsize=9, color='#444')

# ---- Panneau droit: état du tape ----
ax2 = axes[1]
ax2.set_xlim(0, 10); ax2.set_ylim(0, 10); ax2.axis('off')
ax2.set_facecolor('#fafafa')
ax2.text(5, 9.5, 'Tape (construit ligne par ligne)', ha='center', fontsize=11, fontweight='bold')
...
En revanche, `jnp.sin(tracer)` est une opération que JAX connaît. JAX intercepte l'appel, enregistre l'opération sur le tape, calcule la valeur concrète, et retourne un nouveau traceur. La trace reste intacte.
...
class Value:
    """Scalaire différentiable: encapsule un flottant et construit le tape."""
