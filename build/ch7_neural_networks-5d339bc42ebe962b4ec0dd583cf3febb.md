...
for ax, traj, title, color in zip(
    axes,
    [traj_sgd, traj_mom],
    ['SGD (zigzags)', r'SGD + Momentum ($\beta=0{,}9$)'],
    ['#1f77b4', '#d62728']
):
    ax.contourf(T1, T2, Z, levels=20, cmap='Greys', alpha=0.5)
    ax.contour( T1, T2, Z, levels=20, colors='gray', linewidths=0.4, alpha=0.6)
    # Trajectoire colorée par progression
    for k in range(len(traj)-1):
        alpha = 0.3 + 0.7*(k / (len(traj)-1))
        ax.plot(traj[k:k+2, 0], traj[k:k+2, 1], '-', color=color, lw=1.8, alpha=alpha)
    ax.plot(*theta0, 'ko', ms=8, zorder=5, label='Départ')
    ax.plot(0, 0, 'r*', ms=12, zorder=5, label='Minimum')
    ax.plot(*traj[-1], 's', color=color, ms=8, zorder=5, label='Arrivée')
...
class MLP:
    ...
    def __init__(self, n_input, n_hidden, n_output,
        ...
        self._s = {k: np.zeros_like(v) for k in self._m}
