...
La propriété la plus distinctive de la convolution est son **équivariance à la translation**. Définissons l'opérateur de translation sur un signal discret: $[T_\tau x][n] = x[n - \tau]$, qui décale le signal de $\tau$ positions. La convolution commute avec la translation:

$$
\text{conv}(w, T_\tau x) = T_\tau \, \text{conv}(w, x)
$$ (eq:equivariance)

En d'autres termes: convoluer puis translater donne le même résultat que translater puis convoluer. Si un détecteur de contour produit une forte réponse à la position $(i, j)$ et que l'image est décalée de $(\Delta i, \Delta j)$, la réponse se déplace exactement du même décalage.

Avec un MLP, ce n'est pas le cas. Translater l'image d'entrée de quelques pixels produit un vecteur complètement différent du point de vue de la couche dense, et les activations changent de manière imprévisible.
...
axes[0, 1].set_title(f'Signal translaté $T_{{{tau}}} x$')
...
La réponse du noyau se déplace avec le signal, sans changer de forme. C'est l'équivariance en action: la convolution ne perd pas l'information de position, elle la préserve.
...
Revenons aux images. Les noyaux de traitement d'image classiques révèlent différents aspects de la structure visuelle.
...
2. Donnez un exemple d'opération qui est **invariante** à la translation (la sortie ne change pas du tout lorsque l'entrée est translatée).
