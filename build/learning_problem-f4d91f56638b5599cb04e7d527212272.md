...
### De la vraisemblance à la prédiction

Une question naturelle se pose: nous apprenons une fonction déterministe $f(\mathbf{x}; \boldsymbol{\theta})$, mais nous l'avons intégrée dans un modèle probabiliste. Au moment de faire une prédiction pour un nouveau $\mathbf{x}$, devons-nous échantillonner de la gaussienne, ou utiliser directement $f(\mathbf{x})$?

La réponse dépend de ce que nous cherchons à faire.
...
Le prédicteur optimal est la **moyenne conditionnelle**. Dans notre modèle gaussien, $\mathbb{E}[y|\mathbf{x}] = f(\mathbf{x}; \boldsymbol{\theta})$. Donc, pour une prédiction ponctuelle, nous utilisons directement $f(\mathbf{x})$, sans échantillonner.
...
#### Pourquoi alors un modèle probabiliste?
...
3. **Comparer des modèles**: La vraisemblance permet de comparer différents modèles sur les mêmes données (via des critères comme l'AIC ou le BIC).

4. **Générer des données synthétiques**: Si nous voulons simuler des données, nous pouvons échantillonner $y \sim \mathcal{N}(f(\mathbf{x}), \sigma^2)$.

En résumé, le modèle probabiliste guide l'**apprentissage** (quelle fonction objectif optimiser) et permet la **quantification d'incertitude**, mais pour une prédiction ponctuelle, nous utilisons simplement la moyenne conditionnelle.
