...
Nous ne voyons que les points à droite. La structure continue à gauche — les contours, les densités, les régions de haute et basse probabilité — nous est cachée. C'est à partir de ces quelques points que nous devons estimer la performance de notre modèle.
...
**Pour les espaces continus**: L'intégrale est une intégrale de dimension élevée. Si $x \in \mathbb{R}^d$ avec $d$ grand (par exemple, $d = 1000$ pour des images ou $d = 10^6$ pour des données textuelles), nous devons intégrer sur un espace de dimension $d+1$. Les méthodes d'intégration numérique (comme la quadrature de Gauss) deviennent exponentiellement coûteuses en dimension: pour une précision donnée, le nombre de points d'évaluation nécessaires croît exponentiellement avec la dimension. C'est ce qu'on appelle la "malédiction de la dimensionnalité".
...
En dimension 10, il faut déjà $10^{10}$ points — dix milliards. En dimension 100, il en faut $10^{100}$, un nombre qui dépasse le nombre d'atomes dans l'univers observable. L'intégration numérique directe est donc impossible pour les problèmes de haute dimension, même si nous connaissions $p(x, y)$ exactement.
...
ax.plot(degrees, train_errors, 'o-', label='Erreur entraînement')
ax.plot(degrees, test_errors, 's-', label='Erreur test')
