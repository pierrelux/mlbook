...
4. **Le cadre probabiliste**: Le cadre bayésien fournit une perspective unifiée. Le maximum de vraisemblance (EMV) et le maximum a posteriori (MAP) sont deux approches concrètes, l'EMV étant un cas particulier du MAP avec a priori uniforme. Sous bruit gaussien, l'EMV coïncide avec les moindres carrés.

5. **Régularisation**: Pénaliser la complexité du modèle équivaut à ajouter un a priori sur les paramètres. La régression ridge correspond au MAP avec a priori gaussien.

6. **Classes de modèles**: Le choix de la classe de fonctions détermine ce que le modèle peut représenter. L'expansion de caractéristiques permet aux modèles linéaires de capturer des relations non linéaires.
