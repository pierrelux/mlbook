...
# Vue 1: Espace des directions principales (coordonnées dans la base V)
ax1 = fig.add_subplot(1, 2, 1)

# Dessiner les axes des directions principales
...
# Solutions Ridge
for lam, color in zip(lambdas[1:], colors[1:]):
    # Calculer dans l'espace des directions principales
    shrinkage = d_svd**2 / (d_svd**2 + lam)
    theta_ridge_coords = shrinkage * theta_ols_coords
    theta_ridge = V_svd @ theta_ridge_coords
    
    ax1.plot(theta_ridge[0], theta_ridge[1], 'o', color=color, markersize=8, 
             label=f'Ridge ($\\lambda={lam}$)', zorder=4)
    ax1.text(theta_ridge[0]+0.15, theta_ridge[1]+0.15, 
             f'$\\lambda={lam}$', fontsize=10, ha='left', color=color)
    
    # Flèche montrant le rétrécissement
    ax1.arrow(theta_ols[0], theta_ols[1], 
             theta_ridge[0] - theta_ols[0], theta_ridge[1] - theta_ols[1],
...
ax1.set_title('Rétrécissement dans l\'espace des coefficients', fontsize=13)
ax1.legend(loc='upper right', fontsize=10)
ax1.grid(True, alpha=0.3)
ax1.set_aspect('equal')
ax1.axis([-1, 3.5, -1, 2.5])

# Vue 2: Espace des directions principales (coordonnées dans la base V)
ax2 = fig.add_subplot(1, 2, 2)

# Coordonnées dans la base V (directions principales)
theta_ols_v = theta_ols_coords  # Coordonnées dans la base V

# Calculer toutes les solutions Ridge pour déterminer les limites
all_points_v = [theta_ols_v]
for lam in lambdas[1:]:
    shrinkage = d_svd**2 / (d_svd**2 + lam)
    theta_ridge_v = shrinkage * theta_ols_v
    all_points_v.append(theta_ridge_v)

all_points_v = np.array(all_points_v)
x_min, x_max = all_points_v[:, 0].min() - 0.3, all_points_v[:, 0].max() + 0.5
y_min, y_max = all_points_v[:, 1].min() - 0.3, all_points_v[:, 1].max() + 0.5

# Dessiner les axes (proportionnels aux limites)
...
# Solutions Ridge dans l'espace V
for lam, color in zip(lambdas[1:], colors[1:]):
    shrinkage = d_svd**2 / (d_svd**2 + lam)
    theta_ridge_v = shrinkage * theta_ols_v
    
    ax2.plot(theta_ridge_v[0], theta_ridge_v[1], 'o', color=color, markersize=8, 
             label=f'Ridge ($\\lambda={lam}$)', zorder=4)
    
    # Flèche montrant le rétrécissement
    dx = theta_ridge_v[0] - theta_ols_v[0]
    dy = theta_ridge_v[1] - theta_ols_v[1]
    if abs(dx) > 0.01 or abs(dy) > 0.01:  # Seulement si le déplacement est significatif
        ax2.arrow(theta_ols_v[0], theta_ols_v[1], dx, dy,
                 head_width=abs(x_max-x_min)*0.02, head_length=abs(x_max-x_min)*0.02, 
                 fc=color, ec=color, alpha=0.6, linewidth=2, linestyle='--')
    
    # Annoter le facteur de rétrécissement pour chaque direction
    # Direction v1 (grande valeur singulière)
    shrink1 = d_svd[0]**2 / (d_svd[0]**2 + lam)
    mid_x1 = (theta_ols_v[0] + theta_ridge_v[0]) / 2
    mid_y1 = theta_ols_v[1] + 0.15*abs(y_max-y_min)
    ax2.text(mid_x1, mid_y1, f'$\\frac{{d_1^2}}{{d_1^2+{lam}}}={shrink1:.2f}$', 
             fontsize=8, ha='center', va='bottom', color=color,
             bbox=dict(boxstyle='round,pad=0.2', facecolor='white', alpha=0.7, edgecolor=color))
    
    # Direction v2 (petite valeur singulière)
    shrink2 = d_svd[1]**2 / (d_svd[1]**2 + lam)
    mid_x2 = theta_ols_v[0] + 0.15*abs(x_max-x_min)
    mid_y2 = (theta_ols_v[1] + theta_ridge_v[1]) / 2
    ax2.text(mid_x2, mid_y2, f'$\\frac{{d_2^2}}{{d_2^2+{lam}}}={shrink2:.2f}$', 
...
ax2.set_title('Rétrécissement dans l\'espace des directions principales', fontsize=13)
...
La figure de gauche montre le rétrécissement dans l'espace des coefficients originaux $(\theta_1, \theta_2)$. La solution MCO (point noir) est "tirée" vers l'origine par la régularisation Ridge (flèches pointillées). La figure de droite montre le même phénomène dans l'espace des directions principales $(\mathbf{v}_1, \mathbf{v}_2)$ de la SVD. On voit que la direction $\mathbf{v}_2$ (associée à la petite valeur singulière $d_2$) est plus fortement rétrécie que $\mathbf{v}_1$ (grande valeur singulière $d_1$), car le facteur $\frac{d_2^2}{d_2^2 + \lambda}$ est plus petit que $\frac{d_1^2}{d_1^2 + \lambda}$.
