...
Dans cet exemple, le point bleu (carré) est plus loin en distance euclidienne, mais plus proche en distance de Mahalanobis — car il se trouve dans la direction où les données varient naturellement. Le point orange (triangle), bien que plus proche en euclidien, est "surprenant" par rapport à la distribution et donc plus loin en Mahalanobis.
...
Les k-ppv fonctionnent bien en basse dimension mais souffrent en haute dimension. Ce phénomène, le **fléau de la dimensionnalité**, affecte toutes les méthodes basées sur la localité. Le problème n'est pas principalement calculatoire — le coût $O(Nd)$ croît linéairement en $d$ — mais fondamentalement *statistique*.
