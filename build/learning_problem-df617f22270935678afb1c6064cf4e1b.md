...
#### Obstacle 1: La distribution $p(x, y)$ est inconnue
...
Supposons, par un miracle, que nous connaissions exactement $p(x, y)$. Pourrions-nous alors calculer le risque $\mathcal{R}(f) = \int \ell(y, f(x)) \, p(x, y) \, dx \, dy$?
...
En dimension 10, il faut déjà $10^{10}$ points, soit dix milliards. En dimension 100, il en faut $10^{100}$, un nombre qui dépasse le nombre d'atomes dans l'univers observable. L'intégration numérique directe est donc impossible pour les problèmes de haute dimension, même si nous connaissions $p(x, y)$ exactement.

### Le risque empirique comme seule option pratique

Face à ces obstacles, le risque empirique est notre seule option calculable. Il remplace l'intégrale sur la distribution inconnue par une moyenne sur l'échantillon fini que nous possédons:
...
Imaginons que nous observons les distances de freinage de notre véhicule. Chaque mesure est légèrement différente, même à la même vitesse. Cette variabilité suggère un modèle: la distance observée est la distance "vraie" plus un bruit aléatoire. Si nous supposons que ce bruit suit une loi gaussienne, nous avons un modèle probabiliste complet de la génération des données.

Maintenant, posons-nous la question inverse: *étant donné les données que nous avons observées, quels paramètres auraient le plus probablement produit ces observations?*

C'est exactement ce que le maximum de vraisemblance calcule.

### La vraisemblance

Supposons que notre modèle définisse une distribution conditionnelle $p(y|x; \theta)$ paramétrée par $\theta$. Pour chaque exemple $(x_i, y_i)$, cette distribution nous dit à quel point l'observation $y_i$ est "probable" étant donné $x_i$ et les paramètres $\theta$.

La **vraisemblance** est la probabilité conjointe d'avoir observé toutes nos données sous le modèle:

$$
\mathcal{L}(\theta) = p(\mathcal{D}|\theta) = \prod_{i=1}^N p(y_i | x_i; \theta)
$$

Le produit découle de l'hypothèse que les exemples sont tirés indépendamment. Cette quantité mesure la "compatibilité" entre les paramètres $\theta$ et les données observées. Des paramètres qui assignent une probabilité élevée aux observations sont plus vraisemblables.

L'**estimateur du maximum de vraisemblance** (EMV) est simplement le $\theta$ qui maximise cette compatibilité:
...
ax.set_title(f'$\sigma$ estimé: {sigma_mle:.2f} mg/L')
...
```{code-cell} python
from mlbook_code.datasets import make_tool_wear
