...
2. **Rétrécissement des coefficients** (*shrinkage*): Comme nous l'avons vu dans la section sur la SVD, la solution MCO s'écrit:

$$
\hat{\boldsymbol{\theta}}_{\text{MCO}} = \sum_{j=1}^d \frac{\mathbf{u}_j^\top \mathbf{y}}{d_j} \mathbf{v}_j
$$

où chaque terme $\frac{\mathbf{u}_j^\top \mathbf{y}}{d_j} \mathbf{v}_j$ représente la contribution de la direction principale $\mathbf{v}_j$ à la solution. La régularisation Ridge modifie cette formule en remplaçant le facteur $1/d_j$ par $\frac{d_j^2}{d_j^2 + \lambda} \cdot \frac{1}{d_j} = \frac{d_j}{d_j^2 + \lambda}$:
...
# Vue 1: Espace des coefficients originaux
ax1 = fig.add_subplot(1, 2, 1)

# Dessiner les axes des directions principales (statiques)
v1, v2 = V_svd[:, 0], V_svd[:, 1]
ax1.arrow(0, 0, v1[0]*3, v1[1]*3, head_width=0.15, head_length=0.15, 
          fc='gray', ec='gray', alpha=0.5, linewidth=1.5)
ax1.arrow(0, 0, v2[0]*3, v2[1]*3, head_width=0.15, head_length=0.15, 
          fc='gray', ec='gray', alpha=0.5, linewidth=1.5)
ax1.text(v1[0]*3.3, v1[1]*3.3, '$\\mathbf{v}_1$ ($d_1$ grand)', 
         fontsize=11, ha='center', color='gray')
ax1.text(v2[0]*3.3, v2[1]*3.3, '$\\mathbf{v}_2$ ($d_2$ petit)', 
         fontsize=11, ha='center', color='gray')
...
title1 = ax1.set_title('Rétrécissement dans l\'espace des coefficients', fontsize=13)
...
# Vue 2: Espace des directions principales
ax2 = fig.add_subplot(1, 2, 2)

# Coordonnées dans la base V
theta_ols_v = theta_ols_coords

# Calculer les limites pour différentes valeurs de lambda
lambda_max = 5.0
shrinkage_max = d_svd**2 / (d_svd**2 + lambda_max)
theta_ridge_v_max = shrinkage_max * theta_ols_v
all_points_v = np.array([theta_ols_v, theta_ridge_v_max])
x_min, x_max = all_points_v[:, 0].min() - 0.3, all_points_v[:, 0].max() + 0.5
y_min, y_max = all_points_v[:, 1].min() - 0.3, all_points_v[:, 1].max() + 0.5

# Dessiner les axes (statiques)
axis_scale = max(abs(x_max), abs(x_min), abs(y_max), abs(y_min)) * 0.8
ax2.arrow(0, 0, axis_scale, 0, head_width=axis_scale*0.03, head_length=axis_scale*0.03, 
          fc='gray', ec='gray', alpha=0.5, linewidth=1.5)
ax2.arrow(0, 0, 0, axis_scale, head_width=axis_scale*0.03, head_length=axis_scale*0.03, 
          fc='gray', ec='gray', alpha=0.5, linewidth=1.5)
ax2.text(axis_scale*1.1, 0, f'$\\mathbf{{v}}_1$ ($d_1={d_svd[0]:.1f}$)', 
         fontsize=11, ha='left', color='gray', va='center')
ax2.text(0, axis_scale*1.1, f'$\\mathbf{{v}}_2$ ($d_2={d_svd[1]:.1f}$)', 
         fontsize=11, ha='center', color='gray')

# Solution OLS (statique)
ax2.plot(theta_ols_v[0], theta_ols_v[1], 'ko', markersize=10, label='MCO', zorder=5)
ax2.text(theta_ols_v[0]+0.1*abs(x_max-x_min), theta_ols_v[1]+0.1*abs(y_max-y_min), 
         '$\\hat{\\boldsymbol{\\theta}}_{\\text{MCO}}$', fontsize=12, ha='left')

# Point Ridge (animé)
point2, = ax2.plot([], [], 'ro', markersize=8, label='Ridge', zorder=4)
arrow2 = ax2.annotate('', xy=(0, 0), xytext=(0, 0), 
                      arrowprops=dict(arrowstyle='->', color='red', 
                                     linestyle='--', linewidth=2, alpha=0.6))
title2 = ax2.set_title('Rétrécissement dans l\'espace des directions principales', fontsize=13)
lambda_text2 = ax2.text(0.02, 0.98, '', transform=ax2.transAxes, 
                        fontsize=11, va='top', ha='left',
                        bbox=dict(boxstyle='round', facecolor='white', alpha=0.8))
shrink_text = ax2.text(0.02, 0.90, '', transform=ax2.transAxes, 
                       fontsize=9, va='top', ha='left',
                       bbox=dict(boxstyle='round', facecolor='white', alpha=0.8))

ax2.set_xlabel('Coordonnée dans $\\mathbf{v}_1$', fontsize=12)
ax2.set_ylabel('Coordonnée dans $\\mathbf{v}_2$', fontsize=12)
...
def animate(frame):
    ...
    theta_ridge_v = theta_ridge_coords
    
    # Mettre à jour le point dans l'espace des coefficients
    ...
    # Mettre à jour le point dans l'espace des directions principales
    point2.set_data([theta_ridge_v[0]], [theta_ridge_v[1]])
    arrow2.xy = (theta_ridge_v[0], theta_ridge_v[1])
    arrow2.xytext = (theta_ols_v[0], theta_ols_v[1])
    lambda_text2.set_text(f'$\\lambda = {lam:.2f}$')
    
    # Afficher les facteurs de rétrécissement
    shrink1 = d_svd[0]**2 / (d_svd[0]**2 + lam)
    shrink2 = d_svd[1]**2 / (d_svd[1]**2 + lam)
    shrink_text.set_text(f'$\\frac{{d_1^2}}{{d_1^2+\\lambda}}={shrink1:.2f}$\n$\\frac{{d_2^2}}{{d_2^2+\\lambda}}={shrink2:.2f}$')
    
    return point1, point2, arrow1, arrow2, lambda_text1, lambda_text2, shrink_text

# Créer l'animation
anim = FuncAnimation(fig, animate, frames=100, interval=50, blit=True, repeat=True)
anim.save('_static/ridge_shrinkage.gif', writer='pillow', fps=20, dpi=100)
plt.close()

# Afficher le GIF
Image(filename='_static/ridge_shrinkage.gif')
```

L'animation montre le rétrécissement continu lorsque $\lambda$ augmente de 0 à 5. La figure de gauche illustre le mouvement dans l'espace des coefficients originaux $(\theta_1, \theta_2)$: la solution MCO (point noir) est progressivement "tirée" vers l'origine. La figure de droite montre le même phénomène dans l'espace des directions principales $(\mathbf{v}_1, \mathbf{v}_2)$ de la SVD. On observe que la direction $\mathbf{v}_2$ (associée à la petite valeur singulière $d_2 = 0.5$) est plus fortement rétrécie que $\mathbf{v}_1$ (grande valeur singulière $d_1 = 5.0$), car le facteur $\frac{d_2^2}{d_2^2 + \lambda}$ décroît plus rapidement que $\frac{d_1^2}{d_1^2 + \lambda}$ lorsque $\lambda$ augmente. Les annotations montrent les valeurs numériques de ces facteurs de rétrécissement en temps réel.
...
La solution MCO peut également être calculée en utilisant la **décomposition en valeurs singulières** (SVD) de la matrice $\mathbf{X}$. Cette approche offre une interprétation géométrique et des avantages numériques.
...
En substituant cette décomposition dans la solution MCO:

$$
\hat{\boldsymbol{\theta}}_{\text{MCO}} = (\mathbf{X}^\top \mathbf{X})^{-1} \mathbf{X}^\top \mathbf{y} = (\mathbf{V} \mathbf{D}^2 \mathbf{V}^\top)^{-1} \mathbf{V} \mathbf{D} \mathbf{U}^\top \mathbf{y}
$$

En utilisant $(\mathbf{V} \mathbf{D}^2 \mathbf{V}^\top)^{-1} = \mathbf{V} \mathbf{D}^{-2} \mathbf{V}^\top$ et en simplifiant:
...
Cette formule révèle l'interprétation géométrique de la solution:

1. **$\mathbf{U}^\top \mathbf{y}$**: Projette le vecteur des observations $\mathbf{y}$ sur les directions $\mathbf{u}_j$ dans l'espace des observations. Chaque composante $(\mathbf{u}_j^\top \mathbf{y})$ mesure la contribution de la direction $j$ aux données.

2. **$\mathbf{D}^{-1}$**: Inverse les valeurs singulières. Si une valeur singulière $d_j$ est petite, son inverse $1/d_j$ est grand, ce qui amplifie les erreurs de mesure dans cette direction. C'est la source de l'instabilité numérique quand $\mathbf{X}$ est mal conditionnée.

3. **$\mathbf{V}$**: Transforme de l'espace des directions principales vers l'espace des coefficients originaux. Les colonnes $\mathbf{v}_j$ définissent les **directions principales** dans l'espace des paramètres.
...
**Interprétation géométrique**: Les directions principales $\mathbf{v}_j$ définissent les axes d'une ellipse de confiance dans l'espace des coefficients. Les longueurs des demi-axes sont proportionnelles à $1/d_j$: les directions associées aux grandes valeurs singulières (fort signal) ont des axes courts (faible incertitude), tandis que celles associées aux petites valeurs singulières (faible signal) ont des axes longs (grande incertitude).
