...
2. **Minimisation du risque empirique**: Le principe fondamental consiste à choisir le modèle qui minimise le risque empirique. La généralisation, la validation croisée et le choix des hyperparamètres permettent de contrôler l'écart entre risque empirique et vrai risque.

3. **Régularisation**: Pénaliser la complexité du modèle dans la fonction objectif permet de contrôler le surapprentissage. La régression ridge ajoute une pénalité $\ell_2$ sur les paramètres.

4. **Le cadre probabiliste**: Le cadre bayésien fournit une perspective unifiée. Le maximum de vraisemblance (EMV) et le maximum a posteriori (MAP) sont deux approches concrètes. Sous bruit gaussien, l'EMV coïncide avec les moindres carrés; la régularisation $\ell_2$ correspond au MAP avec a priori gaussien.

5. **Classes de modèles**: Le choix de la classe de fonctions détermine ce que le modèle peut représenter. L'expansion de caractéristiques permet aux modèles linéaires de capturer des relations non linéaires.
