...
Dans cette section, nous voyons pourquoi EM maximise une borne de la log-vraisemblance, puis nous plaçons EM dans le cadre plus général de l'**inférence variationnelle**. Cette partie est un peu plus abstraite; si vous êtes en première lecture, retenir que « EM maximise l'ELBO en alternant sur $q$ et $\boldsymbol{\theta}$ » suffit pour la suite.

Nous voulons maximiser $\log p(\mathbf{X} \mid \boldsymbol{\theta})$ pour un modèle à variables latentes (par exemple un GMM, avec $\mathbf{X}$ les observations et $\mathbf{Z}$ les assignations de composant). La difficulté vient de la marginalisation: $\log p(\mathbf{X} \mid \boldsymbol{\theta}) = \log \sum_{\mathbf{Z}} p(\mathbf{X}, \mathbf{Z} \mid \boldsymbol{\theta})$, et la somme (ou l'intégrale) à l'intérieur du logarithme empêche d'optimiser directement. EM contourne le problème en alternant l'étape E et l'étape M. Nous allons dériver la borne inférieure dont il a été question en marge (l'ELBO), puis montrer que EM en est précisément la maximisation par alternance.

### La borne inférieure de l'évidence (ELBO)

Pour toute distribution $q(\mathbf{Z})$ sur les variables latentes (avec $q(\mathbf{Z}) > 0$ là où $p(\mathbf{X}, \mathbf{Z} \mid \boldsymbol{\theta}) > 0$), on peut écrire la log-vraisemblance comme suit. Par définition de l'a posteriori $p(\mathbf{Z} \mid \mathbf{X}, \boldsymbol{\theta}) = p(\mathbf{X}, \mathbf{Z} \mid \boldsymbol{\theta}) / p(\mathbf{X} \mid \boldsymbol{\theta})$:

$$
\log p(\mathbf{X} \mid \boldsymbol{\theta}) = \log \frac{p(\mathbf{X}, \mathbf{Z} \mid \boldsymbol{\theta})}{p(\mathbf{Z} \mid \mathbf{X}, \boldsymbol{\theta})}.
$$

En prenant l'espérance sous $q(\mathbf{Z})$ (qui ne dépend pas de $\boldsymbol{\theta}$), on a:

$$
\log p(\mathbf{X} \mid \boldsymbol{\theta}) = \mathbb{E}_{q(\mathbf{Z})}\left[ \log \frac{p(\mathbf{X}, \mathbf{Z} \mid \boldsymbol{\theta})}{p(\mathbf{Z} \mid \mathbf{X}, \boldsymbol{\theta})} \right].
$$

On ajoute et soustrait $\log q(\mathbf{Z})$ dans l'espérance, puis on sépare en deux termes:

$$
\log p(\mathbf{X} \mid \boldsymbol{\theta}) = \mathbb{E}_{q(\mathbf{Z})}\left[ \log p(\mathbf{X}, \mathbf{Z} \mid \boldsymbol{\theta}) - \log q(\mathbf{Z}) \right] + \mathbb{E}_{q(\mathbf{Z})}\left[ \log \frac{q(\mathbf{Z})}{p(\mathbf{Z} \mid \mathbf{X}, \boldsymbol{\theta})} \right].
$$

Le premier terme est la **borne inférieure de l'évidence** (en anglais *Evidence Lower Bound*, ELBO). Le second est la divergence de Kullback-Leibler $D_{\mathrm{KL}}(q(\mathbf{Z}) \,\|\, p(\mathbf{Z} \mid \mathbf{X}, \boldsymbol{\theta}))$, qui est toujours positive ou nulle. Donc:

$$
\log p(\mathbf{X} \mid \boldsymbol{\theta}) = \mathrm{ELBO}(q, \boldsymbol{\theta}) + D_{\mathrm{KL}}(q(\mathbf{Z}) \,\|\, p(\mathbf{Z} \mid \mathbf{X}, \boldsymbol{\theta})), \qquad \mathrm{ELBO}(q, \boldsymbol{\theta}) = \mathbb{E}_{q(\mathbf{Z})}\left[ \log p(\mathbf{X}, \mathbf{Z} \mid \boldsymbol{\theta}) - \log q(\mathbf{Z}) \right].
$$

Comme $D_{\mathrm{KL}} \geq 0$, on obtient $\log p(\mathbf{X} \mid \boldsymbol{\theta}) \geq \mathrm{ELBO}(q, \boldsymbol{\theta})$, avec égalité si et seulement si $q(\mathbf{Z}) = p(\mathbf{Z} \mid \mathbf{X}, \boldsymbol{\theta})$ (alors la KL est nulle). Maximiser l'ELBO par rapport à $q$ et $\boldsymbol{\theta}$ donne donc une procédure qui pousse une borne inférieure de la log-vraisemblance vers le haut.

### Maximiser l'ELBO et le lien avec EM

En **inférence variationnelle**, on maximise l'ELBO par rapport au couple $(q, \boldsymbol{\theta})$ en restreignant $q$ à une **famille variationnelle** $\mathcal{Q}$ (par exemple des distributions factorisées, ou paramétrées par un réseau). On choisit $q \in \mathcal{Q}$ et $\boldsymbol{\theta}$ pour rendre l'ELBO le plus grand possible.

Dans le cas du GMM, l'a posteriori $p(\mathbf{Z} \mid \mathbf{X}, \boldsymbol{\theta})$ est calculable en closed form: ce sont exactement les responsabilités $r_{nk}$ que nous avons utilisées. Si on ne restreint pas $q$ (ou si la famille $\mathcal{Q}$ contient toutes les distributions), alors pour $\boldsymbol{\theta}$ fixé, le $q$ qui maximise l'ELBO est celui qui rend la KL nulle, soit $q^*(\mathbf{Z}) = p(\mathbf{Z} \mid \mathbf{X}, \boldsymbol{\theta})$. On peut donc faire une maximisation par **coordonnées**:

1. À $\boldsymbol{\theta}$ fixé, maximiser l'ELBO en $q$: le maximum est atteint pour $q^* = p(\mathbf{Z} \mid \mathbf{X}, \boldsymbol{\theta})$, ce qui correspond à l'**étape E** (calcul des responsabilités).
2. À $q$ fixée (égale à cet a posteriori), maximiser l'ELBO en $\boldsymbol{\theta}$: on obtient les formules de l'**étape M** (moyennes pondérées, etc.).

Ainsi, EM est le cas particulier de l'inférence variationnelle où la famille variationnelle n'est pas restreinte et où l'a posteriori est tractable: à chaque itération, l'étape E choisit la « meilleure » $q$ possible (l'a posteriori), et l'étape M met à jour $\boldsymbol{\theta}$. Comme l'ELBO augmente (ou reste constant) à chaque demi-pas, la log-vraisemblance augmente aussi, ce qui justifie la convergence vers un maximum local.

```{admonition} Quand l'a posteriori est intractable (optionnel IFT3395)
:class: note

Dans de nombreux modèles (modèles de thèmes, autoencodeurs variationnels, etc.), l'a posteriori $p(\mathbf{Z} \mid \mathbf{X}, \boldsymbol{\theta})$ n'est pas calculable analytiquement. On restreint alors $q$ à une famille tractable (par exemple gaussienne factorisée, ou paramétrée par un réseau) et on maximise l'ELBO par descente de gradient ou par alternance. Ces extensions sont abordées dans des chapitres ou cours dédiés aux modèles génératifs profonds; le lecteur curieux pourra consulter le draft sur les modèles graphiques pour une présentation de l'inférence variationnelle par gradient et des autoencodeurs variationnels.
```
