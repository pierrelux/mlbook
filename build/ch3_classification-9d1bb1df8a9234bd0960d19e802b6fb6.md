...
où $\sigma$ est la fonction sigmoïde que nous connaissons bien. Un score plus élevé signifie une meilleure réponse, mais la préférence reste probabiliste: même une réponse légèrement moins bonne peut parfois être choisie. Ces scores sont dits latents car nous ne les observons pas directement: nous ne voyons que les résultats des comparaisons, et nous devons inférer les scores à partir de ces observations.
...
Chaque ligne de $\mathbf{X}$ encode une comparaison: un $+1$ pour le premier objet comparé, un $-1$ pour le second, et des zéros ailleurs. L'étiquette $y = 1$ indique que le premier objet a gagné, $y = 0$ que le second a gagné. La régression logistique sur cette matrice estime les scores $\mathbf{s} = (s_0, s_1, s_2, s_3)^\top$ qui maximisent la vraisemblance des résultats observés.
