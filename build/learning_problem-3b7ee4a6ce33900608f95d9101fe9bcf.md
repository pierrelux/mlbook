...
Il est important de voir $\mathcal{D}$ comme une **variable aléatoire**: si nous répétions l'expérience de collecte de données, nous obtiendrions un échantillon différent. Cette perspective, adoptée notamment dans {cite:t}`murphy2022probabilistic`, est fondamentale: elle nous rappelle que nos conclusions dépendent de l'échantillon particulier que nous avons observé. C'est comme si nous regardions quelques gouttes d'eau d'un océan: nous pouvons analyser ces gouttes, mais un autre prélèvement donnerait des gouttes différentes.
...
$$
\hat{\mathcal{R}}(\boldsymbol{\theta}) = \frac{1}{N} \sum_{i=1}^N -\log p(y_i | \mathbf{x}_i; \boldsymbol{\theta}) = \frac{1}{N} \text{NLV}(\boldsymbol{\theta})
$$
...
L'expansion de caractéristiques ne sert pas qu'à capturer des relations polynomiales en régression. Elle joue un rôle fondamental en **classification**: transformer des données non linéairement séparables en données linéairement séparables. Pour développer cette intuition, commençons par un exemple simple en une dimension.
...
L'animation révèle l'idée centrale: **l'expansion de caractéristiques "déplie" la géométrie des données**. Vue de dessus, la structure est celle des cercles concentriques originaux. Mais la troisième dimension $z = x_1^2 + x_2^2$ sépare verticalement les deux classes: le disque central reste bas, l'anneau extérieur monte. Un plan horizontal (en vert) suffit alors à séparer les classes.
...
Ces exemples illustrent un principe fondamental: **en augmentant la dimension de l'espace de représentation, nous augmentons les chances que les données deviennent linéairement séparables**.

Pour deux classes de $N$ points en position générale, un résultat classique de géométrie affirme qu'elles sont presque sûrement séparables par un hyperplan si la dimension de l'espace est au moins $N$. C'est pourquoi l'expansion polynomiale de haut degré, ou d'autres expansions vers des espaces de grande dimension, permettent souvent d'obtenir une séparation parfaite sur les données d'entraînement.

Mais cette flexibilité a un coût: plus l'espace est grand, plus le modèle risque de mémoriser les particularités des données d'entraînement plutôt que d'apprendre la structure sous-jacente. C'est le **compromis biais-variance** que nous avons observé avec les polynômes de haut degré. La section suivante présente une technique pour contrôler ce compromis: la régularisation.
...
Comparons avec la solution OLS: $\hat{\boldsymbol{\theta}}_{\text{MCO}} = (\mathbf{X}^\top \mathbf{X})^{-1} \mathbf{X}^\top \mathbf{y}$. La seule différence est l'ajout du terme $\lambda \mathbf{I}$ à la matrice $\mathbf{X}^\top \mathbf{X}$.
...
Une question fondamentale se pose: une fois que nous avons appris un modèle probabiliste $p(y|\mathbf{x}, \mathcal{D})$, comment l'utiliser pour faire des prédictions? Devons-nous échantillonner de cette distribution, ou existe-t-il une prédiction "optimale"?

Supposons que nous connaissions la vraie distribution conditionnelle $p(y|\mathbf{x})$. Quelle prédiction $\hat{y}$ minimise notre perte espérée? C'est le **prédicteur de Bayes optimal**:

$$
f^*(\mathbf{x}) = \arg\min_{\hat{y}} \mathbb{E}_{p(y|\mathbf{x})}[\ell(y, \hat{y})]
$$

Pour la **perte quadratique**, on peut montrer que:

$$
f^*(\mathbf{x}) = \arg\min_{\hat{y}} \mathbb{E}[(y - \hat{y})^2 | \mathbf{x}] = \mathbb{E}[y | \mathbf{x}]
$$

Le prédicteur optimal est la **moyenne conditionnelle**. Dans un modèle gaussien où $p(y|\mathbf{x}) = \mathcal{N}(y | f(\mathbf{x}; \boldsymbol{\theta}), \sigma^2)$, nous avons $\mathbb{E}[y|\mathbf{x}] = f(\mathbf{x}; \boldsymbol{\theta})$. Pour une prédiction ponctuelle, nous utilisons donc directement $f(\mathbf{x})$, sans échantillonner.

Pour la **perte 0-1** en classification, le prédicteur optimal est le **mode conditionnel**:

$$
f^*(\mathbf{x}) = \arg\max_c \, p(y = c | \mathbf{x})
$$

C'est la classe la plus probable. Là encore, pas besoin d'échantillonner pour prédire.

#### Utilité du modèle probabiliste

Si nous n'échantillonnons pas pour prédire, à quoi sert le modèle probabiliste? Il remplit plusieurs rôles:

1. **Justifier la fonction de perte**: La perte quadratique n'est pas un choix arbitraire. Elle découle naturellement de l'hypothèse de bruit gaussien via le maximum de vraisemblance.

2. **Quantifier l'incertitude**: Au-delà de la prédiction ponctuelle $\hat{y} = f(\mathbf{x})$, nous pouvons donner un **intervalle de prédiction**. Sous le modèle gaussien, $y$ a environ 95% de chances de tomber dans $[f(\mathbf{x}) - 2\sigma, f(\mathbf{x}) + 2\sigma]$.

3. **Comparer des modèles**: La vraisemblance permet de comparer différents modèles sur les mêmes données $\mathcal{D}$ via des critères comme le critère d'information d'Akaike (AIC) ou le critère d'information bayésien (BIC).

4. **Générer des données synthétiques**: Pour simuler des données, nous pouvons échantillonner $y \sim \mathcal{N}(f(\mathbf{x}), \sigma^2)$.

En résumé, le modèle probabiliste guide l'**apprentissage** (quelle fonction objectif optimiser) et permet la **quantification d'incertitude**, mais pour une prédiction ponctuelle, nous utilisons la moyenne conditionnelle.
...
Dans ce cas, $\log p(\boldsymbol{\theta})$ est une constante qui n'affecte pas l'optimisation, et MAP se réduit à MLE:
...
1. **Risque et risque empirique**: Le risque mesure la performance que nous voulons optimiser; le risque empirique est son approximation calculable à partir des données.

2. **Minimisation du risque empirique**: Le principe fondamental consiste à choisir le modèle qui minimise le risque empirique, en espérant que cela minimise aussi le vrai risque.

3. **Généralisation**: L'écart entre risque empirique et vrai risque peut mener au surapprentissage ou au sous-apprentissage. La validation croisée permet d'estimer cet écart et de choisir les hyperparamètres.

4. **Maximum de vraisemblance**: Une perspective complémentaire fondée sur l'inférence statistique. Sous bruit gaussien, l'EMV coïncide avec les moindres carrés.

5. **Régularisation et interprétation bayésienne**: La régularisation correspond à l'ajout d'un a priori sur les paramètres. L'EMV est un cas particulier du MAP avec a priori uniforme.

Le chapitre suivant développe les outils théoriques pour quantifier précisément quand et comment le risque empirique prédit le vrai risque.
