...
$$
p(y | \mathbf{x}; \mathbf{w}, b, \sigma^2) = \mathcal{N}(y | \mathbf{w}^\top \mathbf{x} + b, \sigma^2)
$$

La fonction de moyenne est linéaire: $f_\mu(\mathbf{x}; \mathbf{w}, b) = \mathbf{w}^\top \mathbf{x} + b = \sum_{j=1}^d w_j x_j + b$, où $\mathbf{w} \in \mathbb{R}^d$ est le vecteur de poids et $b \in \mathbb{R}$ est le biais (ou ordonnée à l'origine). La variance $\sigma^2$ est constante.
...
$$
\mathbf{X} = \begin{pmatrix} 1 & x_{11} & x_{12} & \cdots & x_{1d} \\ 1 & x_{21} & x_{22} & \cdots & x_{2d} \\ \vdots & \vdots & \vdots & \ddots & \vdots \\ 1 & x_{N1} & x_{N2} & \cdots & x_{Nd} \end{pmatrix}, \quad \mathbf{y} = \begin{pmatrix} y_1 \\ y_2 \\ \vdots \\ y_N \end{pmatrix}, \quad \mathbf{w} = \begin{pmatrix} b \\ w_1 \\ \vdots \\ w_d \end{pmatrix}
$$

Le vecteur des prédictions est $\hat{\mathbf{y}} = \mathbf{X}\mathbf{w}$, et le vecteur des résidus est $\mathbf{r} = \mathbf{y} - \mathbf{X}\mathbf{w}$.

La **somme des carrés des résidus** (*residual sum of squares*, RSS) s'écrit:

$$
\text{RSS}(\mathbf{w}) = \sum_{i=1}^N (y_i - \mathbf{w}^\top \mathbf{x}_i)^2 = \|\mathbf{y} - \mathbf{X}\mathbf{w}\|_2^2 = (\mathbf{y} - \mathbf{X}\mathbf{w})^\top (\mathbf{y} - \mathbf{X}\mathbf{w})
$$
...
Pour trouver le minimum, nous calculons le gradient du RSS par rapport à $\mathbf{w}$ et l'égalons à zéro. En développant:
...
$$
\nabla_{\mathbf{w}} \text{RSS}(\mathbf{w}) = -2\mathbf{X}^\top \mathbf{y} + 2\mathbf{X}^\top \mathbf{X} \mathbf{w}
$$
...
Les paramètres sont le vecteur de poids $\mathbf{w} \in \mathbb{R}^d$ et le biais $b \in \mathbb{R}$. Ce modèle suppose que la sortie est une combinaison linéaire des entrées.
...
La prédiction devient $f(x; \mathbf{w}) = \mathbf{w}^\top \phi(x)$. Le modèle est polynomial en $x$ mais linéaire en $\mathbf{w}$, ce qui permet d'utiliser les mêmes algorithmes d'optimisation.
...
$$
\hat{\mathbf{w}}_{\text{ridge}} = \arg\min_{\mathbf{w}} \frac{1}{N}\sum_{i=1}^N (y_i - \mathbf{w}^\top \mathbf{x}_i)^2 + \lambda \|\mathbf{w}\|_2^2
$$
...
$$
\text{RSS}_\lambda(\mathbf{w}) = \|\mathbf{y} - \mathbf{X}\mathbf{w}\|_2^2 + \lambda \|\mathbf{w}\|_2^2
$$

En développant et en calculant le gradient:

$$
\nabla_{\mathbf{w}} \text{RSS}_\lambda(\mathbf{w}) = -2\mathbf{X}^\top \mathbf{y} + 2\mathbf{X}^\top \mathbf{X} \mathbf{w} + 2\lambda \mathbf{w} = -2\mathbf{X}^\top \mathbf{y} + 2(\mathbf{X}^\top \mathbf{X} + \lambda \mathbf{I}) \mathbf{w}
$$
...
Comparons avec la solution OLS: $\hat{\mathbf{w}}_{\text{OLS}} = (\mathbf{X}^\top \mathbf{X})^{-1} \mathbf{X}^\top \mathbf{y}$. La seule différence est l'ajout du terme $\lambda \mathbf{I}$ à la matrice $\mathbf{X}^\top \mathbf{X}$.
...
$$
\hat{\mathbf{w}}_{\text{ridge}} = \sum_{j=1}^d \frac{d_j^2}{d_j^2 + \lambda} \frac{\mathbf{u}_j^\top \mathbf{y}}{d_j} \mathbf{v}_j
$$
...
3. **Stabilité numérique**: Quand $\mathbf{X}^\top \mathbf{X}$ est presque singulière, de petites perturbations dans les données causent de grandes variations dans $\hat{\mathbf{w}}_{\text{OLS}}$. La régularisation réduit cette sensibilité.
...
### Régression ridge comme MAP

Appliquons maintenant ce cadre bayésien à la régression linéaire. Si nous plaçons un a priori gaussien isotrope sur les poids:

$$
p(\mathbf{w}) = \mathcal{N}(\mathbf{w} | \mathbf{0}, \sigma_w^2 \mathbf{I})
$$

cet a priori exprime la croyance que les poids sont probablement proches de zéro, avec une incertitude contrôlée par $\sigma_w^2$.

Le logarithme négatif de cet a priori est:

$$
-\log p(\mathbf{w}) = \frac{1}{2\sigma_w^2} \|\mathbf{w}\|_2^2 + \text{constante}
$$

L'estimateur MAP devient:

$$
\hat{\mathbf{w}}_{\text{MAP}} = \arg\min_{\mathbf{w}} \left[ \text{NLL}(\mathbf{w}) + \frac{1}{2\sigma_w^2}\|\mathbf{w}\|_2^2 \right]
$$

C'est exactement la régression ridge, avec $\lambda = 1/(2\sigma_w^2)$. Cette correspondance nous donne une interprétation de l'hyperparamètre:

- **Grande valeur de $\lambda$** (petite variance $\sigma_w^2$): forte croyance que les poids sont proches de zéro
- **Petite valeur de $\lambda$** (grande variance $\sigma_w^2$): a priori peu informatif, on fait confiance aux données

L'a priori gaussien sur les poids est parfois appelé **dégradation des poids** (*weight decay*) dans le contexte des réseaux de neurones, car il "tire" les poids vers zéro pendant l'entraînement.

### Choix de l'hyperparamètre

La valeur de $\lambda$ (ou de manière équivalente, $\sigma_w^2$) est un **hyperparamètre** qui doit être choisi avant l'entraînement. Comment le choisir?
