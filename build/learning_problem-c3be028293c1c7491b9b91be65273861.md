...
np.random.seed(42)
n, d = 50, 10
# Créer une matrice avec des valeurs singulières décroissantes
U, _ = np.linalg.qr(np.random.randn(n, d))
V, _ = np.linalg.qr(np.random.randn(d, d))
# Valeurs singulières décroissantes (certaines très petites)
d_vals = np.array([10.0, 8.0, 6.0, 4.0, 2.0, 0.8, 0.3, 0.1, 0.05, 0.01])
D = np.diag(d_vals)
X = U @ D @ V.T
...
# Calculer les coefficients OLS (via SVD)
U_svd, d_svd, Vt_svd = np.linalg.svd(X, full_matrices=False)
theta_ols = Vt_svd.T @ np.diag(1 / d_svd) @ U_svd.T @ y
...
# Graphique 1: Facteur de rétrécissement en fonction des valeurs singulières
ax1 = axes[0]
d_j = d_svd
for lam in lambdas:
    shrinkage = d_j**2 / (d_j**2 + lam)
    label = 'MCO ($\\lambda=0$)' if lam == 0 else f'Ridge ($\\lambda={lam}$)'
    ax1.plot(d_j, shrinkage, 'o-', label=label, linewidth=2, markersize=6)

ax1.axhline(1.0, color='gray', linestyle='--', alpha=0.5, linewidth=1)
ax1.set_xlabel('Valeur singulière $d_j$')
ax1.set_ylabel('Facteur de rétrécissement $\\frac{d_j^2}{d_j^2 + \\lambda}$')
ax1.set_title('Effet de rétrécissement: les petites valeurs singulières sont plus fortement pénalisées')
ax1.legend(loc='best')
ax1.grid(True, alpha=0.3)
ax1.set_xscale('log')

# Graphique 2: Coefficients OLS vs Ridge
ax2 = axes[1]
x_pos = np.arange(len(d_svd))
width = 0.35

# Trier par valeur singulière décroissante pour la clarté
sort_idx = np.argsort(d_svd)[::-1]
d_sorted = d_svd[sort_idx]
theta_ols_sorted = theta_ols[sort_idx]

# Calculer les coefficients Ridge pour lambda=1
lam_show = 1.0
theta_ridge = Vt_svd.T @ np.diag(d_sorted / (d_sorted**2 + lam_show)) @ U_svd.T @ y
theta_ridge_sorted = theta_ridge[sort_idx]

x = np.arange(len(d_sorted))
ax2.bar(x - width/2, theta_ols_sorted, width, label='MCO ($\\lambda=0$)', alpha=0.7)
ax2.bar(x + width/2, theta_ridge_sorted, width, label=f'Ridge ($\\lambda={lam_show}$)', alpha=0.7)

ax2.axhline(0, color='black', linewidth=0.8)
ax2.set_xlabel('Composante (triée par $d_j$ décroissant)')
ax2.set_ylabel('Coefficient $\\theta_j$')
ax2.set_title(f'Rétrécissement des coefficients: Ridge réduit tous les coefficients, surtout ceux associés aux petites $d_j$')
ax2.legend(loc='best')
ax2.grid(True, alpha=0.3, axis='y')
ax2.set_xticks(x)
ax2.set_xticklabels([f'$d_{{{i+1}}}={d_sorted[i]:.2f}$' for i in range(len(d_sorted))], rotation=45, ha='right')

plt.tight_layout()
```

La figure supérieure montre le facteur de rétrécissement $\frac{d_j^2}{d_j^2 + \lambda}$ en fonction de la valeur singulière $d_j$. Pour $\lambda = 0$ (MCO), le facteur vaut 1: aucun rétrécissement. Pour $\lambda > 0$, le facteur est toujours inférieur à 1, et il décroît plus rapidement pour les petites valeurs singulières. La figure inférieure compare les coefficients MCO et Ridge: tous les coefficients sont réduits, mais ceux associés aux petites valeurs singulières (composantes de droite) sont plus fortement pénalisés.
