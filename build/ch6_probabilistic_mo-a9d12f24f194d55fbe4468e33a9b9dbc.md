...
**Étape E.** On calcule les responsabilités — la probabilité a posteriori que l'expert $k$ soit responsable de $(\mathbf{x}_n, y_n)$:
...
Le mélange d'experts illustre comment la machinerie EM — responsabilités, moyennes pondérées, alternance E/M — s'adapte au cadre supervisé. Les mêmes idées (variable latente, partitionnement souple, spécialisation des composants) se retrouvent dans les architectures modernes de réseaux de neurones, où les couches MoE permettent d'augmenter la capacité d'un modèle sans augmenter proportionnellement le coût de calcul.
