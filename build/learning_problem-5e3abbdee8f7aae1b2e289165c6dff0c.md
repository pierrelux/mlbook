...
La régularisation $\ell_2$ admet une interprétation probabiliste. Si nous plaçons un a priori gaussien sur les paramètres $p(\mathbf{w}) = \mathcal{N}(\mathbf{w} | \mathbf{0}, \sigma_w^2 \mathbf{I})$, l'estimateur du maximum a posteriori (MAP) est:
...
$$
\hat{\mathbf{w}}_{\text{MAP}} = \arg\min_{\mathbf{w}} \text{NLL}(\mathbf{w}) + \frac{1}{2\sigma_w^2}\|\mathbf{w}\|_2^2
$$
