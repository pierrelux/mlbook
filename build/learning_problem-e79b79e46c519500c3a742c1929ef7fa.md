...
- **$\mathbf{V}$** contient les **directions principales** dans l'espace des caractéristiques (les colonnes $\mathbf{v}_j$ sont orthonormales). Ces directions correspondent aux axes principaux de variation des données. Elles sont identiques aux directions données par l'analyse en composantes principales (ACP) si vous l'avez vue.
- **$\mathbf{D}$** est une matrice diagonale contenant les **valeurs singulières** $d_1 \geq d_2 \geq \cdots \geq d_d \geq 0$, ordonnées du plus grand au plus petit. Chaque valeur singulière $d_j$ mesure l'**étendue** de la variation le long de la direction $\mathbf{v}_j$. Une grande valeur singulière signifie que les données s'étirent beaucoup dans cette direction; une petite valeur singulière signifie que la variation est faible (les données sont presque constantes le long de cette direction).
