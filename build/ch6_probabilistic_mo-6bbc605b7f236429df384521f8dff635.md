...
K-moyennes est rapide et simple, mais il a une limite structurelle: la distance euclidienne traite toutes les directions de la même manière, ce qui produit des frontières de décision perpendiculaires au segment reliant deux centroïdes. Les groupes retrouvés sont donc nécessairement **sphériques**. Si les données contiennent des groupes allongés ou d'orientations variées, k-moyennes les découpe mal.

```{code-cell} python
:tags: [hide-input]

# Illustration: frontières de k-moyennes = médiatrices entre centroïdes
from matplotlib.patches import Ellipse

np.random.seed(42)

# Données avec deux groupes elliptiques
cov_a = np.array([[3.0, 1.8], [1.8, 1.5]])
cov_b = np.array([[1.0, -0.7], [-0.7, 2.5]])
X_a = np.random.multivariate_normal([-1, -1], cov_a, 120)
X_b = np.random.multivariate_normal([3, 3], cov_b, 120)
...
# Gauche: frontière de k-moyennes avec médiatrice
ax = axes[0]
colors_2 = ['steelblue', 'coral']
for k in range(2):
    mask = labels_f == k
    ax.scatter(X_ell[mask, 0], X_ell[mask, 1], c=colors_2[k], alpha=0.4, s=20)
    ax.plot(mu_f[k, 0], mu_f[k, 1], marker='X', color=colors_2[k],
            markersize=14, markeredgecolor='black', markeredgewidth=1.5, zorder=5)

# Tracer le segment entre centroïdes et la médiatrice
mid = (mu_f[0] + mu_f[1]) / 2
diff = mu_f[1] - mu_f[0]
...
ax.plot([mu_f[0, 0], mu_f[1, 0]], [mu_f[0, 1], mu_f[1, 1]],
        'k--', linewidth=1.5, alpha=0.5, label='Segment entre centroïdes')
t = np.linspace(-6, 6, 100)
boundary_x = mid[0] + t * perp[0]
boundary_y = mid[1] + t * perp[1]
in_range = (boundary_x > -5) & (boundary_x < 7) & (boundary_y > -5) & (boundary_y < 7)
ax.plot(boundary_x[in_range], boundary_y[in_range], 'k-', linewidth=2.5,
        label='Frontière (médiatrice)')
ax.plot(mid[0], mid[1], 'ko', markersize=6, zorder=5)

ax.set_xlabel('$x_1$')
ax.set_ylabel('$x_2$')
ax.set_title('Frontière de k-moyennes')
...
# Droite: les vrais groupes elliptiques
ax = axes[1]
for k, (c, X_group, mu_group, cov_group) in enumerate([
        ('steelblue', X_a, [-1, -1], cov_a), ('coral', X_b, [3, 3], cov_b)]):
    ax.scatter(X_group[:, 0], X_group[:, 1], c=c, alpha=0.4, s=20)
    vals, vecs = np.linalg.eigh(cov_group)
    angle = np.degrees(np.arctan2(vecs[1, 0], vecs[0, 0]))
    for n_std in [1, 2]:
        w, h = 2 * n_std * np.sqrt(vals)
        ell = Ellipse(mu_group, w, h, angle=angle, fill=False, color=c,
...
La figure montre le problème. À gauche, la frontière de k-moyennes est la **médiatrice** du segment reliant les deux centroïdes: une droite perpendiculaire passant par le milieu. Cette frontière est identique quelle que soit la forme des groupes. À droite, les vrais groupes sont des ellipses inclinées qui chevauchent cette frontière. Pour s'adapter à cette géométrie, il faut un modèle capable de représenter des covariances différentes par groupe.
...
Le lien entre les deux est direct. Supposons que tous les composants partagent une covariance sphérique $\boldsymbol{\Sigma}_k = \sigma^2 \mathbf{I}$ et des poids uniformes $\pi_k = 1/K$. Les responsabilités se réduisent alors à un softmax sur les distances:
