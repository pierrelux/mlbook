...
Commençons par un idéal théorique. Si nous connaissions la **vraie** distribution conditionnelle $p(y|\mathbf{x})$ (celle de la nature, pas un modèle appris), quelle prédiction minimiserait notre perte espérée? La réponse dépend de la fonction de perte choisie. Le **prédicteur de Bayes optimal** pour une perte $\ell$ est:

$$
f^*(\mathbf{x}) = \arg\min_{\hat{y}} \mathbb{E}_{p(y|\mathbf{x})}[\ell(y, \hat{y})]
$$

Pour la **perte quadratique** $\ell(y, \hat{y}) = (y - \hat{y})^2$, le prédicteur optimal est la **moyenne conditionnelle**: $f^*(\mathbf{x}) = \mathbb{E}[y | \mathbf{x}]$. Pour la **perte 0-1** en classification $\ell(y, \hat{y}) = \mathbf{1}[y \neq \hat{y}]$, c'est le **mode conditionnel**: $f^*(\mathbf{x}) = \arg\max_c \, p(y = c | \mathbf{x})$, la classe la plus probable. Chaque perte définit son propre prédicteur optimal.
