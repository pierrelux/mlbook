...
Cette section est plus technique que les précédentes. Nous y dérivons l'algorithme de rétropropagation pas à pas. Si les détails des jacobiennes semblent abstraits au premier abord, concentrez-vous sur l'exemple travaillé de la section suivante qui rend ces idées concrètes.
...
Si vous pouvez suivre cet exemple du début à la fin, vous avez compris le mécanisme de la rétropropagation. L'algorithme général ci-dessous ne fait que formaliser cette procédure pour un nombre arbitraire de couches.
