...
Les démos ci-dessous sont des jeux de données réels ou publics bien adaptés à la régression logistique (binaire puis multiclasse). Chaque démo indique où télécharger les données, la cible à prédire et pourquoi le problème se prête à ce chapitre.

**Progression recommandée** : les démos 1 à 3 se chargent en une ligne avec `ucimlrepo` et ne demandent aucun prétraitement pour une démo de base. On peut les enchaîner dans l’ordre : Béton (déjà vu en régression) → Plaques d’acier (données industrielles) → Supraconducteur (découverte de matériaux).

### 1. Béton : Pass / Fail (résistance)

**Contexte** : Prédire si un béton « passe » ou « échoue » un critère de résistance à partir de sa formulation et de son âge. Les étudiants connaissent déjà ce jeu de données du chapitre sur la régression (résistance continue).

**Chargement** : `from ucimlrepo import fetch_ucirepo` puis récupération du jeu *Concrete Compressive Strength*. Une cible binaire s’obtient en seuillant la résistance (ex. passe = 1 si résistance $\geq$ seuil, 0 sinon).

**Pourquoi c’est adapté** : Lien direct régression → classification (même données, cible binaire au lieu de continue). Interprétation des coefficients (influence de chaque ingrédient sur la probabilité de passer).

### 2. Plaques d’acier : défauts (contrôle qualité industriel)

**Contexte** : Classifier les défauts sur des plaques d’acier à partir de mesures géométriques et physiques. Cas typique de contrôle qualité en fabrication.

**Chargement** : une ligne avec `ucimlrepo` (jeu *Steel Plates Faults*).

**Cible binaire simple** : « Rayure » (*Scratch*) vs « non rayure » (fusionner les autres classes). Ensuite, **multiclasse** : type de défaut (Pastry, Z_Scratch, K_Scratch, Stains, Dirtiness, Bumps, Other).

**Pourquoi c’est adapté** : Données industrielles réelles que les étudiants retrouveront en contexte fabrication. Interprétation physique des coefficients (quelles variables augmentent la probabilité de tel défaut). Idéal pour enchaîner régression logistique binaire puis softmax multiclasse.

### 3. Supraconducteur : High-Tc vs Low-Tc (découverte de matériaux)

**Contexte** : Prédire si un matériau est supraconducteur à haute température critique (High-Tc) ou à basse (Low-Tc) à partir de descripteurs chimiques (formule, éléments, etc.). Connexion directe avec la recherche en découverte de matériaux.

**Chargement** : une ligne avec `ucimlrepo` (jeu *Superconductor*). Créer une cible binaire en seuillant la température critique (ex. High-Tc si $T_c \geq$ 30 K, Low-Tc sinon).

**Pourquoi c’est adapté** : Classification binaire sur des caractéristiques réelles (composition, nombre d’électrons, etc.). Montre l’usage de la régression logistique en sciences des matériaux et pour le criblage de candidats.

### 4. LBNL HVAC Fault Detection Dataset (HVAC / contrôles)

**Contexte** : Classifier les états de fonctionnement de systèmes CVC (chauffage, ventilation, climatisation) comme « normal » ou « en défaut » à partir de mesures de capteurs — centrales de froid (*chiller*), centrales de traitement d’air (AHU), unités terminales (RTU).

**Source** : [LBNL HVAC Fault Detection, OpenEI](https://data.openei.org/submissions/5763)  
**Systèmes** : Centrales de froid (*chiller*), AHU, RTU, chaudières, ventilo-convecteurs.  
**Cible** : Binaire : normal vs défaut (ou multiclasse : type de défaut).  
**Caractéristiques** : Températures, pressions, débits, positions de vannes, puissance électrique.

**Pourquoi c’est adapté** :

- Proche des applications CVC / bâtiment (e.g. Trane, BrainBox).
- Données de bâtiments réels, pas de simulation pure.
- Plusieurs types de défauts : biais de capteur, vanne bloquée, encrassement de batterie, sous-charge en fluide frigorigène.
- **Étape 1 (binaire)** : « Y a-t-il un défaut ? » → régression logistique binaire.
- **Étape 2 (multiclasse)** : « Quel type de défaut ? » → régression logistique multiclasse (softmax).
- **Aspect temporel** : séries temporelles ; on peut agréger les variables sur une fenêtre glissante (moyenne, écart-type, etc.) pour obtenir des entrées par fenêtre et appliquer la régression logistique sur ces résumés.

Après téléchargement et prétraitement (nettoyage, création des étiquettes binaires ou multiclasses, agrégation par fenêtre si besoin), on obtient une matrice $\mathbf{X}$ et des étiquettes $y$. On peut alors réutiliser l’implémentation SGD du chapitre (ou `sklearn.linear_model.LogisticRegression`) pour entraîner un modèle binaire puis un modèle multiclasse sur les types de défauts.
