...
# ACP: projection linéaire sur la première composante principale
from numpy.linalg import eigh
X_centered = X - X.mean(axis=0)
cov = X_centered.T @ X_centered / N
eigenvalues, eigenvectors = eigh(cov)
# La plus grande valeur propre est la dernière (eigh trie par ordre croissant)
w1 = eigenvectors[:, -1]
z_pca = X_centered @ w1
...
# Auto-encodeur non linéaire (entraîné manuellement avec un réseau simple)
# On utilise un petit MLP: 2 -> 64 -> 1 -> 64 -> 2
...
W1 = rng.randn(2, 64) * np.sqrt(2.0 / 2)
b1 = np.zeros(64)
W2 = rng.randn(64, 1) * np.sqrt(2.0 / 64)
b2 = np.zeros(1)
W3 = rng.randn(1, 64) * np.sqrt(2.0 / 1)
b3 = np.zeros(64)
W4 = rng.randn(64, 2) * np.sqrt(2.0 / 64)
b4 = np.zeros(2)

lr = 0.001
X_train = X_centered.copy()

for epoch in range(3000):
    # Avant
    h1 = X_train @ W1 + b1
    a1 = relu(h1)
    z_ae = a1 @ W2 + b2           # code latent (N, 1)
    h3 = z_ae @ W3 + b3
    a3 = relu(h3)
    x_hat = a3 @ W4 + b4          # reconstruction (N, 2)

    # Perte
    diff = x_hat - X_train        # (N, 2)

    # Arrière
    dL_dxhat = 2.0 * diff / N
    dL_dW4 = a3.T @ dL_dxhat
    dL_db4 = dL_dxhat.sum(axis=0)
    dL_da3 = dL_dxhat @ W4.T
    dL_dh3 = dL_da3 * relu_grad(h3)
    dL_dW3 = z_ae.T @ dL_dh3
    dL_db3 = dL_dh3.sum(axis=0)
    dL_dz = dL_dh3 @ W3.T
    dL_dW2 = a1.T @ dL_dz
    dL_db2 = dL_dz.sum(axis=0)
    dL_da1 = dL_dz @ W2.T
    dL_dh1 = dL_da1 * relu_grad(h1)
    dL_dW1 = X_train.T @ dL_dh1
...
h3 = z_ae @ W3 + b3
a3 = relu(h3)
X_ae_recon = a3 @ W4 + b4 + X.mean(axis=0)
