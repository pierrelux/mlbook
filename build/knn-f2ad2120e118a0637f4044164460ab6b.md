...
Pour comprendre comment une norme mesure les distances, on trace sa **boule unité**: l'ensemble des points $x$ tels que $\|x\| \leq 1$. Tous les points sur le bord de cette boule sont à distance exactement 1 de l'origine. La forme de la boule révèle ce que la norme considère comme "équidistant".
...
où $\Sigma$ est la matrice de covariance. Cette distance "déforme" l'espace pour que les directions de forte variance comptent moins. En pratique, on l'utilise rarement directement — la normalisation standard suffit généralement.
