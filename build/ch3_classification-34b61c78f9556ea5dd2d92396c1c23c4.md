...
- Reconnaître le modèle de Bradley-Terry comme une régression logistique sur une matrice de design particulière
...
Vous avez peut-être entendu parler du RLHF (*Reinforcement Learning from Human Feedback*), la technique utilisée pour aligner les grands modèles de langage comme ChatGPT sur les préférences humaines. Au coeur de cette approche se trouve un modèle de préférences que nous pouvons comprendre entièrement avec les outils de ce chapitre: le modèle de Bradley-Terry n'est rien d'autre qu'une régression logistique sur une matrice de design particulière.

Imaginons que des annotateurs comparent des paires de réponses générées par un modèle de langage. Pour chaque paire $(A, B)$, ils indiquent laquelle ils préfèrent. Comment transformer ces jugements qualitatifs en un signal quantitatif? L'idée est d'attribuer un score $s_A$ à chaque réponse $A$ et de modéliser la probabilité que $A$ soit préférée à $B$ par

$$
P(A \succ B) = \sigma(s_A - s_B) = \frac{1}{1 + e^{-(s_A - s_B)}}
$$

où $\sigma$ est la fonction sigmoïde que nous connaissons bien. Un score plus élevé signifie une meilleure réponse, mais la préférence reste probabiliste: même une réponse légèrement moins bonne peut parfois être choisie.
...
Cette formulation est exactement une régression logistique. Pour le voir, supposons que nous avons $K$ objets à comparer (des réponses, des joueurs d'échecs, des produits). Chaque objet $k$ possède un score latent $s_k$ que nous voulons estimer. Si nous observons le résultat d'une comparaison entre les objets $i$ et $j$, nous pouvons construire un vecteur $\mathbf{x}_{ij} \in \mathbb{R}^K$ qui vaut $+1$ à la position $i$, $-1$ à la position $j$, et $0$ partout ailleurs. Le produit scalaire avec le vecteur de scores donne alors $\mathbf{s}^\top \mathbf{x}_{ij} = s_i - s_j$, exactement la différence qui entre dans la sigmoïde.

Prenons un exemple concret avec quatre joueurs. Pour le match «joueur 0 contre joueur 2», le vecteur d'entrée est $\mathbf{x} = (1, 0, -1, 0)$. Si nous empilons tous ces vecteurs pour $N$ matchs observés, nous obtenons une matrice $\mathbf{X}$ de taille $N \times K$ où chaque ligne contient un seul $+1$, un seul $-1$, et des zéros ailleurs. La régression logistique sur cette matrice estime les scores $\mathbf{s}$ qui maximisent la vraisemblance des résultats observés.
...
# Générer des matchs et construire la matrice de design
n_matches = 200
X = []
y = []
...
- Le modèle de Bradley-Terry pour les préférences est une régression logistique sur une matrice de design où chaque ligne encode une comparaison par paires. C'est le modèle au coeur du RLHF pour l'alignement des grands modèles de langage.
