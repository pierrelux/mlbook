...
flute_preview = comparisons[comparisons['instrument'] == 'flute'][['field1', 'field2', 'win1', 'tie', 'win2']].copy()
flute_preview['field1'] = flute_preview['field1'].apply(lambda x: f'{x:03d}')
flute_preview['field2'] = flute_preview['field2'].apply(lambda x: f'{x:03d}')
print("Aperçu des comparaisons (flûte uniquement):")
print(flute_preview.head(10).to_string(index=False))
```

Les données contiennent 84 comparaisons: 28 paires possibles entre les 8 champs sonores, répétées pour trois instruments (violon, violoncelle, flûte). Pour chaque paire, on enregistre combien de fois le premier champ a été préféré (`win1`), combien de fois il y a eu égalité (`tie`), et combien de fois le second a été préféré (`win2`). Pour simplifier, nous traitons les égalités en les comptant comme un demi-point pour chaque champ.
...
flute_data['field1'] = flute_data['field1'].apply(lambda x: f'{x:03d}')
flute_data['field2'] = flute_data['field2'].apply(lambda x: f'{x:03d}')
...
Les scores estimés révèlent les préférences des auditeurs. Un score élevé indique un champ sonore apprécié; un score faible indique une configuration moins appréciée. La différence entre deux scores donne le log-odds de préférence: si le champ «100» a un score de 0.8 et le champ «000» un score de -0.5, alors $\sigma(0.8 - (-0.5)) = \sigma(1.3) \approx 0.79$, soit environ 79% de chances que «100» soit préféré à «000».
