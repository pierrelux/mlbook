...
**Panneau central — Paysage de perte**: Les contours gris représentent la fonction de coût RSS. L'ellipse est allongée car les deux caractéristiques $x_1$ et $x_2$ sont fortement corrélées (colinéarité): la RSS varie peu le long de la direction de colinéarité. La solution MCO (point noir) est au centre des ellipses. Le cercle orange représente la contrainte $\|\boldsymbol{\theta}\|_2 \leq c$: plus $\lambda$ est grand, plus le cercle est petit. La solution Ridge (point orange) est le point du cercle qui minimise la RSS, se déplaçant le long du **chemin de régularisation** vers l'origine.
